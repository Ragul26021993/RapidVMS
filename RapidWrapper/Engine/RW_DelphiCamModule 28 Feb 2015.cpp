...
int StartToothStepNumber, BC_Center_C_Dir, BC_CentrePointNumber;
...
RWrapper::RW_DelphiCamModule::RW_DelphiCamModule()
{
	 try
	 {
		 ...
		 HelicalGashParams = gcnew array<double>(4);

		 Hob_RunoutParams = gcnew array<double>(6);
...
bool RWrapper::RW_DelphiCamModule::Start(int MeasurementNo)
{
	...
	{	//CHeck if its already running or if parameters are set or if Axis centre is done; 
		...
		//{
			...
			ProbePath_LstEntity->Pt_Y = ReferenceCenter[1] + HobParameters[4] * MAINDllOBJECT->ProbeRadius(); //cPosition[1];
...
void RWrapper::RW_DelphiCamModule::SendDrotoNextposition()
{
	...
	{	
		...
		if(ProbePath_LstEntity->path && Listcount > 0)
		{
			double shift[4];
		...
		{
			if (givenFeeds[ii] > 0 && givenFeeds[ii] < 21)
...
void RWrapper::RW_DelphiCamModule::Get_Cylinder_Y_Position()
{
	...
	ProbePath_LstEntity->Pt_Y = startPosition[1] + (CylinderDia * 0.4); // 2 + extraDist);
	ProbePath_LstEntity->Pt_Z =  startPosition[2];
	ProbePath_LstEntity->Pt_R =  startPosition[3];
	ProbePath_LstEntity->path = true;
	PointsToGoList->Add(ProbePath_LstEntity);
	...
	ProbePath_LstEntity = gcnew PointsToGo();									
	ProbePath_LstEntity->Pt_X = startPosition[0] + CylinderDia/2;
	ProbePath_LstEntity->Pt_Y = ReferenceCenter[1]; // cPosition[1] - (CylinderDia / 2 + extraDist + 10);// + CylinderDia/2  + extraDist;
	ProbePath_LstEntity->Pt_Z =  cPosition[2];
	ProbePath_LstEntity->Pt_R =  startPosition[3];
	ProbePath_LstEntity->path = false;
	PointsToGoList->Add(ProbePath_LstEntity);
...
bool RWrapper::RW_DelphiCamModule::Run_RunoutCorrection(int StepNumber, int ptsCt, bool Y_Direction)
{
	...
	case 1:
		...
		if (topC->PointsList->Pointscount() > 0)// && NoofShapes == TargetShapeNo)
		{
	...
	case 5:
		...
		ProbePath_LstEntity->Pt_Z = currentZ + (HobParameters[9] + HobParameters[17]) / 4;
...
void RWrapper::RW_DelphiCamModule::CalculatePts_for_C_Axis_Zero()
{
	...
	double initX = ReferenceCenter[0] - (HobParameters[3] / 2) + MAINDllOBJECT->ProbeRadius() + HobParameters[1];//startPosition[0] + MAINDllOBJECT->ProbeRadius() + 20 + HobParameters[1];
	double initY = ReferenceCenter[1] - HobParameters[4] *  MAINDllOBJECT->ProbeRadius();
	ProbePath_LstEntity = gcnew PointsToGo();										
...
void RWrapper::RW_DelphiCamModule::CalculatePts_NoRotation(double Z_Level, int touchDir, bool SkipFirstSteps)
{
	...
	{	
		//Let us move in X and Y to go to the current row
		//Move in Y-axis by a fraction proportional to the number of rows!
		cPosition[0] = startPosition[0] + fracX[j] * (HobParameters[14] / 2 + MAINDllOBJECT->ProbeRadius()) + dirX[j] * extraDist;
...
void RWrapper::RW_DelphiCamModule::Move_To_NextHub(bool inY)
{
	...
	if (inY)
		startPosition[1] = MAINDllOBJECT->TIS_CAxis[1] - (HobParameters[3] / 2) - 10; //+ HobParameters[1];
	else
