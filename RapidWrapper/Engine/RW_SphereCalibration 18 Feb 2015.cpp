...
RWrapper::RW_SphereCalibration::RW_SphereCalibration()
{
	 try
	 {
		 ...
		 CalibratedOffset = gcnew array<double>(12);
		 StartPoint = gcnew array<double>(3); 
		 CurrentSpherePoints = gcnew array<double>(27);
		 CurrentGotoPosition = gcnew array<double>(27);
		 ProbeRadius = gcnew array<double>(5);
		 Target = gcnew array<double>(4);
		 Feedrate = gcnew array<double>(4);
		 ReferenceCenter = gcnew array<double>(4);
		 SphereRadius = 10;
		 PointTakenCount = 0;
		 UserDefinedProbeRadius = 0;
		 ProbeDir = gcnew array<int>(9); 
...
void RWrapper::RW_SphereCalibration::StartSphereCallibration(array<double, 1>^ values, int inspectionType, int currentSide, int currentProbetype, bool circular, bool includeStrt)
{
	try
	{
...
void RWrapper::RW_SphereCalibration::CalibrateCurrentSide()
{
	try
	{
	   ...
	   for(int j = 0; j < 27; j++){ pnts[j] = CurrentSpherePoints[j];}
