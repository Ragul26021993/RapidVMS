...
private:
	...
	bool PorbeActivated, WritingCommands, ReadDroValues, LineNumberResetFlag, VersionControlflag, StopCommunication, EnableInterpolation, HomePositionMode, CanRaiseMachineStuckMessage, SmoothMovementAtProbePos;
	bool ProbeActivated_X, ProbeActivated_Y, ProbeActivated_Z, ProbeActivated_R, SixAxisD33CardisPresent;
	int PositiveAcknowledgementCnt, MachineDisconnect_CycleCount, LimitHit_DroCycleCount;
	int Delaybetweencommnads, ProbDirection, CNCModeCount;
	...
	struct AxisProperties
	{
		...
		int AxisdataLength;
		bool MovementDirection, LashDirection, MovementType, MachineStartflag, LimitSwitchActivated, MoveOppositeToVision;
		std::string ActualCounts, ProbeHexaDecimalValue;
	};
	map<int, AxisProperties> AxisPropCollection;
	
	struct CommandsLogger
	{
		std::string Command;
		std::string time;
	};
	...
	friend void WriteCommandInDiffthread(char* command);
	...
	bool sendMachineCommand_One_NewProtocol2(char* CncCommand);
...
public:
	...
	double GagePosition, GageZeroPosition, PrevGagePosition;
	double DROJumpThreshold;
	std::string ExtraAxesValues[4];

	int Probe_Hit_Direction;
	bool ProbeHitRaiseFlag, GageConnected, A_Axis_isHorizontal;
...
public:
	...
	bool notFocusing, Check_RaxisTargetReached, ActivateNextStepFor_2sPP;
	//Begin Thread to read DigitalReadOut/Handle Callback.
	void beginThread();
	bool ConnectToHardware(bool ReconnectTohardware = false);
	//Number of Axis.......
	...
	void setDROProperties(bool* mt, bool* md, double* lv, double* mf, double* cpu, bool* mov);
	void ResetMachineStartFalg();
	void SetMachineCardFlag(int flag);
	...
	void ColseSoftware();
	bool GetCardVersionNumber();
	//CNC Commands to Controller Functions
	void ActivateCNC();
	void ToAbort();
	void ToManual();
	void ToReset();
	void ResetDROReadings();
	void ResetZeroVal(int axis = -1);
	...
	void SetPorbeRevertbackDistance(int AxisNo, double RevertDist, double RevertSpeed, double ApproachDist, double ApproachSpeed, double ProbeSensitivity, bool ProbeMoveUpFlag);
	//Rotate TIS A Axis
	...
	void FindProbeHitDirection(int pDir_FromController);
	...
	void ConnectToOnlineSystem(bool StopInput);

	void Swivel_A_Axis(); //bool GotoVertical

	void ControlPins(int PinNumber, bool ON_State);
	void SetOneShotLightParams(double pSlope, int pBaseValue, double sSlope, int sBaseValue);
