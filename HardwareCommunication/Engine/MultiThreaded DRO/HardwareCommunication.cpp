...
bool _SendingCommandNow, _AddingCommands, _CommandSent;
...
void DigitalReadOut::beginThread()
{
	try
	{
		WritingCommands = false;
		ReadDroValues = true;
		ProcessDRO_ThreadRunning = true;
		this->HandledAcknowledgement = true;
		//Begin Reading DRO Thread.................
		_beginthread(&ReadingDROValue, 0, this);
		_beginthread(&ProcessDRO_Thread, 0, this);
		_beginthread(&WriteToPort_Thread, 0, this);
	}
...
bool DigitalReadOut::GetCardVersionNumber()
{
	try
	{
		...
		//Code commented on 27 Aug 2016 by R A N becuase its never called. Need to eliminate boolean to update the method of Sending commands. 
...
bool DigitalReadOut::ConnectToHardware(bool ReconnectTohardware)
{
	...
	try
	{
		...
		else
		{
			...
			TimeoutConfigs.ReadIntervalTimeout = MAXDWORD; // 20;
			TimeoutConfigs.ReadTotalTimeoutMultiplier = 10;
			TimeoutConfigs.ReadTotalTimeoutConstant = 0; // 20;
			TimeoutConfigs.WriteTotalTimeoutConstant = 0; // 20;
...
void DigitalReadOut::AxisUnlockAll()
{
	list<std::string> CncCommandList;
	CncCommandList.push_back("B0");
	CncCommandList.push_back("10");
	if (this->newCardFlag < 2)
	{
		CncCommandList.push_back("81");
...
void DigitalReadOut::GotoCommand(double* Target, double* EnteredFRate, bool GotoHomePositionMode, bool probePoint)
{
	try
	{
		...
		std::reverse(AxisTarget.begin(), AxisTarget.end());
		//LPFILETIME cc;
		//GetSystemTimeAsFileTime(cc);
		GetSystemTime(&et);
		//LONGLONG ll_now = (LONGLONG)cc.dwLowDateTime + ((LONGLONG)(ft_now.dwHighDateTime) << 32LL);
		double elapsed = et.wSecond + et.wMilliseconds / 1000 - (st.wSecond + st.wMilliseconds / 1000);
		st = et;
		std::ostringstream ss1;
		ss1 << elapsed;
		WriteCommandSent(ss1.str());
		if (DROInstance->newCardFlag < 2)
		{
			AxisTarget = "5" + AxisTarget;
			CncCommandList.push_back((char*)AxisTarget.c_str());
		...
		GetSystemTime(&et);
		elapsed = et.wSecond + et.wMilliseconds / 1000 - (st.wSecond + st.wMilliseconds / 1000);
		std::ostringstream ss2;
		ss2 << elapsed;
		WriteCommandSent(ss2.str());
	}
...
bool DigitalReadOut::sendLightCommand(char* Lcommand, bool CheckCommunication)
{
	try
	{
		...
		if(DROInstance->newCardFlag > 0)
		{
			...
			_AddingCommands = true;
			DROInstance->CommandsToWrite.push_back(CalculateCRC(FinalLightcommand));
			//DROInstance->HandledAcknowledgement = true;
			_AddingCommands = false;
			return true; // sendLightCommand_NewProtocol(Lcommand, (char*)CalculateCRC(FinalLightcommand).c_str());
		}
		else //if (DROInstance->newCardFlag == 0)
...
bool DigitalReadOut::WriteToPort(std::string TransData)
{
	if (TransData != "@" && TransData != "!")
		TransData = "*" + TransData + "^";
	
	WriteCommandSent("CNC Command :" + TransData);
	int SentCount = 0;
	//TransData = TransData + DROInstance->CalculateCRC(TransData);
	if (DROInstance->newCardFlag < 2)
	{
		for each(char name in TransData)
			if (WriteEachByte(name)) SentCount++;
	}
	else
	{
		for each(char name in TransData)
		{
			if (WriteEachByte2(name)) SentCount++;
		}
	}
	if (SentCount == TransData.size()) return true;
	return false;
}
...
bool DigitalReadOut::sendMachineCommand_One_NewProtocol(char* CncCommand)
{
	...
	try
	{
		...
		DROInstance->CommandsToWrite.push_back(WriteBuffer);

	//SendAgain:
		//while (CommandsSent > 0)
		//{
		...
		//	for each(char name in WriteBuffer)
		//		WriteEachByte2(name);
		//	//int waitforAck = Delaybetweencommnads * 50;
		//	//for (int kkj = 0; kkj < waitforAck; kkj++)
		//	//{
		//	//}
		//}
		//CommandsSent++;
	...
	{
		SetAndRaiseErrorMessage("HCDLL0039", __FILE__, __FUNCSIG__);
	}
	_AddingCommands = false;
	//DROInstance->HandledAcknowledgement = true;
	return returnFlag;
...
//SendAgain:
		...
		//_SendingCommandNow = true;
			for each(std::string WriteBuffer in CncCommandList)
			{
				...
				//	continue;
				//}
				//while (CommandsSent > 0 || DROInstance->PositiveAcknowledgementCnt == 0)
				//{

				//}

				//LastCommandSent = (char*)WriteBuffer.c_str();
				//WriteBuffer = '*' + WriteBuffer + '^';
				//DROInstance->PositiveAcknowledgementCnt = 0;
				//WriteCommandSent("CNC Command :" + WriteBuffer);
				//if (DROInstance->newCardFlag < 2)
				...
				//	for each(char name in WriteBuffer)
				//		WriteEachByte2(name);
				//	//int waitforPositiveAck = Delaybetweencommnads * 20;
				//	//for (int kkj = 0; kkj < waitforPositiveAck; kkj++)
				//	//{
				//	//}
				//}
				//CommandsSent++;
...
bool DigitalReadOut::WriteEachByte(char name)
{
	CheckWrite = false;
	try
	{
		test = &name;
		CheckWrite = WriteFile(PortHandle, test, 1, &WriteResult, NULL);
		//if (DROInstance->newCardFlag < 2)
		//{
		...
		//else
		//{
			//int DelayBetweenC = Delaybetweencommnads * 500000;
			//for (int kkj = 0; kkj < Delaybetweencommnads; kkj++)
			//{
			//}
		//}
	}
...
bool DigitalReadOut::WriteEachByte2(char name)
{
	CheckWrite = false;
	try
	{
		...
		//else
		//{
		//int DelayBetweenC = 250000000;// Delaybetweencommnads * 50000;
		for (int kkj = 0; kkj < Delaybetweencommnads; kkj++)
		{
		}
		//}
...
int SendFailedCount = 0, GotPlusCt = 0;
...
void ReadingDROValue(void* anything)
{
	...
	{	
		...
		bool ErrorWrittentoFile = false, CardStatusFlag = true;
		...
		std::string SpecialChars[] = { "X", "+", "-", "S", "T", "Y" };
		//int SCharIndex[] = { -1, -1, -1, -1, -1 };
		int SCharIndex = -1;
		...
		while(DROInstance->ReadDroValues)
		{
			...
			//If writing command then dont do anything..You can't get any data from controller
				...
				//while(ReadFlag)
				//{
				ReadFile(DROInstance->PortHandle, tempbuffer, 1000, &bufferLength, NULL);
				...
				for (int i = 0; i < 5; i++)
				{
					SCharIndex = str.find_first_of(SpecialChars[i], 0);
					if (SCharIndex >= 0)
					{
						switch (i)
						{
						...
						case 1: //got +
							//Command send was successful!!
							Sleep(15);
							if (_CommandSent && !DROInstance->HandledAcknowledgement) // && DROInstance->CommandsToWrite.size() > 0)
							{
								std::string ss = DROInstance->LastCommandSent;
								DROInstance->PositiveAcknowledgementCnt = 2;
								//if (DROInstance->newCardFlag < 2)
								//	DROInstance->WriteEachByte('@');
								//else
								//	DROInstance->WriteEachByte2('@');
								if (DROInstance->CommandsToWrite.size() > 0) DROInstance->CommandsToWrite.pop_front();
								DROInstance->CommandsToWrite.push_front("@");
								SendFailedCount = 0;
								//DROInstance->PositiveAcknowledgementCnt = 0;
								DROInstance->WriteCommandSent("Recieve     : PositiveAcknowledgement");
								//_SendingCommandNow = false;
								//DROInstance->CommandsSent--
								_CommandSent = false;
								DROInstance->HandledAcknowledgement = true;
							}
							break;
						...
						case 2: //got -
							if (_CommandSent && !_SendingCommandNow && !DROInstance->HandledAcknowledgement) // && DROInstance->CommandsToWrite.size() > 0) // PositiveAcknowledgementCnt == 0)
							{
								if (SendFailedCount < 5)
								{
								...
								{
									DROInstance->WriteCommandSent("Failed (Count is greater than 5): CNC Command");
									DROInstance->ShowMessageBox("HWComm002", false, "");
									if (DROInstance->CommandsToWrite.size() > 0) DROInstance->CommandsToWrite.pop_front();
									SendFailedCount = 0;
								}
								_CommandSent = false;
							...
							//else
							//_beginthread(&WriteCommInDiffthread, 0, DROInstance->LastCommandSent);
							//DROInstance->CommandsToWrite.push_front()
							break;
						...
						default:
							...
							{
								//Check if we are getting continuous Y followed by 8 digits
								if (TempReadBuffer.find("X") == -1 && TempReadBuffer.find("Y") >= 0)
									ReadFlag = false;
								else
				...
				H_Position = TempReadBuffer.find_first_of("H");
				if (H_Position >= 0)
				{
					...
					std::string str1 = TempReadBuffer.substr(H_Position + 1, 1);
					std::string str2 = TempReadBuffer.substr(H_Position + 2, 1);
					if (str1 == "0")
					{
						if (str2 == "1")
						{
							if (!JoystickErrorWrittentoFile)
							{
								...
								DROInstance->WriteCommandSent("Joystick is not connected or not working");
						...
						else if (str2 == "2")
						{
							...
							if (!ErrorWrittentoFile)
							{
								DROInstance->WriteCommandSent("Probe Touch seen when retracting");
								ErrorWrittentoFile = true;
								DROInstance->SetAndRaiseErrorMessage("HCDLL0047_H02", __FILE__, __FUNCSIG__);
								DROInstance->DROTargetReachedCallback(34);
							}
						}
						else if (str2 == "3")
						{
							//UART Input buffer in D33 controller is full
							if (!ErrorWrittentoFile)
							{
								DROInstance->WriteCommandSent("UART input is full");
								ErrorWrittentoFile = true;
								DROInstance->SetAndRaiseErrorMessage("HCDLL0047_H03", __FILE__, __FUNCSIG__);
								//_beginthread(&WriteCommInDiffthread, 0, "E100096");
							}
						}
					}
					else if (str1 == "1")
					{
						if (!ErrorWrittentoFile)
						{
							DROInstance->WriteCommandSent("Axis not communicating proeprly with Central in Axis " + str2);
							ErrorWrittentoFile = true;
							DROInstance->SetAndRaiseErrorMessage("HCDLL0047_H1" + str2, __FILE__, __FUNCSIG__);
						}
					}
					else if (str1 == "2")
					{
						...
						if (CycleCounter > DROInstance->LimitHit_DroCycleCount)
						{
							if (DROInstance->HomePositionMode)
							{
								...
								int limitHitAxis = atoi(str2.c_str()); //TempReadBuffer.substr(2, 1)
							...
							else
							{
								...
								else  //if (DROInstance->newCardFlag < 2)
								{
									...
									{										//raise message machine seems to be stuck....
										DROInstance->WriteCommandSent("Limit Switch hit: " + str2);
										...
										DROInstance->WriteCommandSent(str2);
					...
					{
						DROInstance->WriteCommandSent("Probe hit before target reached");
						//DROInstance->DROTargetReachedCallback(13);
						char cd[4] = "\0";
				...
				else
				{
					...
					ErrorWrittentoFile = false;
				}
				
				if (H_Position < 0) //TempReadBuffer.find("H")
					CycleCounter = 0;
				...
				RBuffer = DROInstance->ReadBuffer;
				...
				//Call the other thread to process the DRO value received
				if (DROInstance->Processing_DRO)
				{
					if (!DROInstance->ProcessDRO_UpdateQueue)
						DROInstance->ProcessDRO_UpdateQueue = true;
				}
				else
					SetEvent(DRO_Semaphore);
...
void WriteCommInDiffthread(void* command)
{
	DROInstance->sendMachineCommand_One((char*)command, true);
	_endthread();
}

void DROCallBackInDiffThread(void* FunctionalValue)
{
	DROInstance->DROTargetReachedCallback((int)FunctionalValue);
	_endthread();
}


void ProcessDRO_Thread(void *anything)
{
	try
	{
		//Create a semaphore for Video graphics thread
		DRO_Semaphore = CreateEvent(NULL, TRUE, FALSE, _T("VideoGraphics"));
		//The graphics thread starts here
		while (DROInstance->ProcessDRO_ThreadRunning)
		{
			//Sleep(0);
			try
			{
				DROInstance->ProcessDRO_Input();
				//WaitForSingleObject(Video_Semaphore, INFINITE); 
				//ResetEvent(Video_Semaphore);
				if (!DROInstance->ProcessDRO_UpdateQueue)
				{
					WaitForSingleObject(DRO_Semaphore, INFINITE);
					ResetEvent(DRO_Semaphore);
				}
				else
				{
					DROInstance->ProcessDRO_UpdateQueue = false;
					DROInstance->Processing_DRO = true;
				}
			}
			catch (...)
			{
				DROInstance->SetAndRaiseErrorMessage("HCDLL0057", __FILE__, __FUNCSIG__);
				_endthread();
			}
		}
		DROInstance->Processing_DRO = false;
		_endthread();
	}
	catch (...)
	{
		DROInstance->SetAndRaiseErrorMessage("HCDLL0057a", __FILE__, __FUNCSIG__);
		_endthread();
	}
}


void DigitalReadOut::ProcessDRO_Input()
{
	try
	{
		std::string RBuffer("");
		RBuffer = DROInstance->ReadBuffer; //Get the current input string to process

		//Probe Related processing...
		size_t Pi = RBuffer.find("{");
		size_t Pj = string::npos;
		if (Pi != string::npos) Pj = RBuffer.substr(Pi).find("}");

		if (Pi != string::npos && Pj != string::npos)
		{
		...
		if (Pi != string::npos && Pj != string::npos && (Pj > Pi))
		{
			if (!DROInstance->AxisPropCollection[0].MachineStartflag && !DROInstance->Probe_HitFlag)
			{
				...
				if (ProbeFlag && !DROInstance->Probe_HitFlag)
				{
					...
					if (!DROInstance->ProbeRetractPointMethod)
					{
						if (DROInstance->newCardFlag == 2)
						{
							std::string HexValue = RBuffer.substr(Pj + 1, 4);
							std::reverse(HexValue.begin(), HexValue.end());
			...
			RBuffer = RBuffer.substr(0, Pi - 1) + RBuffer.substr(Pj + 1);
		}
		...
		while (RBuffer.size() > 0 && RBuffer.size() >= NumberOfChar)
		{
			DroChanged = false;
			...
			if (Line_Number != -1)
			{
				...
				if (DROInstance->LineNumberExecuting > 100) return; // continue;
				...
				if (!DROInstance->LineNumberResetFlag)
				{
							...
							if (DROInstance->LineNumberExecuting == 0)
							{
								...
								if (DROInstance->CheckForTargetReached && DROInstance->CheckPartProgramRunningStatus() && !DROInstance->SmoothMovementAtProbePos)
								{
									...
									else
									{
										...
										DROInstance->CheckForSecondTimeTarget = 0;
										DROInstance->DROTargetReachedCallback(0);
										//_beginthread(&DROCallBackInDiffThread, 0, 0);
									}
								}
								else
								{
					...
					else if (DROInstance->Check_RaxisTargetReached)
					{
							...
							abs(DROInstance->LastTarget[3] - CurrentDROvalue[3]) < DROInstance->Axis_Accuracy[3]))
						{
							...
							//_beginthread(&DROCallBackInDiffThread, 0, (void*)17);
						}
			...
			if (DroChanged)
			{
				...
				DROInstance->DROTargetReachedCallback(10);
				//_beginthread(&DROCallBackInDiffThread, 0, (void*)10);
			...
			if (RaxisValueChanged)
			{
				...
				//_beginthread(&DROCallBackInDiffThread, 0, (void*)11);
			}
			DroChanged = false;
			static int resendcount = 0;
			...
			RBuffer = RBuffer.substr(RBuffer.find_first_of("X", NumberOfChar - 1));
		}
		DROInstance->ReadBuffer = RBuffer;
	}
	catch (...)
	{
...
int EmptyRounds_Count = 0;

void WriteToPort_Thread(void* anything)
{
	try
	{
		...
		while (DROInstance->ProcessDRO_ThreadRunning)
		{
			if (!_CommandSent & !_SendingCommandNow && DROInstance->CommandsToWrite.size() > 0) //) // && !_CommandSent) //
			{
				//if (DROInstance->PositiveAcknowledgementCnt == 0 || DROInstance->PositiveAcknowledgementCnt == 2)
				//{
				if (DROInstance->HandledAcknowledgement)
				{
					_SendingCommandNow = true; //SendFailedCount = 0;
					DROInstance->HandledAcknowledgement = false;
					DROInstance->PositiveAcknowledgementCnt = 0;
					DROInstance->LastCommandSent = DROInstance->CommandsToWrite.front();
					//DataToWrite = DROInstance->LastCommandSent.c_str();

					if (DROInstance->LastCommandSent == "@" || DROInstance->LastCommandSent == "!")
					{
						int ii = 0; 
						while (!_CommandSent && ii < 20)
						{
							_CommandSent = DROInstance->WriteToPort(DROInstance->LastCommandSent);
							//if (!_CommandSent)
							//{
								Sleep(20);
								ii++;
						...
						if (_CommandSent)
						{
							_CommandSent = false;
							DROInstance->HandledAcknowledgement = true;
							DROInstance->CommandsToWrite.pop_front();
						}
					}
					else
						_CommandSent = DROInstance->WriteToPort(DROInstance->LastCommandSent);
				...
				//{
				//	EmptyRounds_Count++;
				//	if (DROInstance->CommandsToWrite.size() > 0 && EmptyRounds_Count > 1000)
				//	{
				//		DROInstance->HandledAcknowledgement = true;
				//		EmptyRounds_Count = 0;
				//	}
				...
				//DROInstance->PositiveAcknowledgementCnt == 0;
			//}
			//else if (DROInstance->PositiveAcknowledgementCnt == 1 && SendFailedCount < 5)
			//{
			//	_SendingCommandNow = true;
			//	DROInstance->sendMachineCommand_One_NewProtocol((char*)(DROInstance->CommandsToWrite.front().c_str()));
			//}
...
void DigitalReadOut::ClearCommandsList()
{
	DROInstance->CommandsToWrite.clear();
}
