...
namespace rrm
{
    public partial class Form1
    {
        ...
        private static IWorkbook workbook, toleranceWorkbook;
        private static ISheet sheet, toleranceSheet;
        private static ICell cell, toleranceCell;
        private static ICellStyle testeStyle, toleranceCellStyleGreen, toleranceCellStyleRed, toleranceCellStyleBorder, toleranceCellStyleBorderRow;
        private static ISheetConditionalFormatting SCF;
        private static IConditionalFormattingRule ColorFormat;
        private static IPatternFormatting FillColor;
        private string DestinationFile, DestinationTemplateFile;
        private string tmpStr;
        private double[] minimumValue = new double[100];
        private double[] maximumValue = new double[100];
        int count = 2, firstColumnInNewPage = 14, colIncrement = 1, WriteAgainColIncrement = 0;
        int tempRowPagination, rowIncrement = 17 , WriteAgainRowIncrement = 0;
        int colNo = 14, rowNo = 42, colNoPag = 0, rowNoPag = 0;
                ...
                if (!WriteAgain)        // The Write Again Signifies that the ReportFile is already created and only Writing on it has to be done.
                {
                    ...
                    ProgramNameNotExists:
                        ...
                        try
                        {
                            ...
                            if (GotDSOFramer)
                            {
                                for (int i = 1, loopTo1 = Conversions.ToInteger(workbook.NumberOfSheets); i <= loopTo1; i++)
                                    _sheetBox.Items.Add(workbook.GetNameAt(i));
                            }
                            else
                            {
                                //xL.InitializeExcelApplication(ReportTemplateName, "");
                                //xL.Open_File(ReportTemplateName);
                                for (int i = 1, loopTo2 = Conversions.ToInteger(workbook.NumberOfSheets); i <= loopTo2; i++)
                                    _sheetBox.Items.Add(workbook.GetNameAt(i));
                                //xL.Close();
                                //xL.ExitExcel();
                            }
        ...
        public bool EnterHobParameterData()
        {
            ...
            try
            {
                ...
                {
                    cell = sheet.GetRow(rLL[i]).GetCell(GetColIndex(cLL[i]));
                    cell.SetCellValue((double)HobPersonalData[i]);
                }
                return true;
            }
            catch (Exception ex)
        ...
        private void _saveButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0, loopTo1 = _extraInformationDataView.Rows.Count - 1; i <= loopTo1; i++)            // Writing the title information into the excel file first. 
                {
                    ...
                    cell = sheet.GetRow(Title_Row).GetCell(GetColIndex(Title_Col));
                    cell.SetCellValue(Title_Name);
                    testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                    cell.CellStyle = testeStyle;
                    // Left Alignment for the title name 
                    ...
                    cell = sheet.GetRow(Title_Row).GetCell(GetColIndex(Col_Next));
                    cell.SetCellValue(Title_Value);
                    testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                    cell.CellStyle = testeStyle;                                                // Right Alignment for the title value 
                }
                ...
                if (chkWriteprogramNameInReport.Checked)                                                       // ==Writing the program Name if the option is checked.
                {
                    ...
                    if (ReportType.Contains("Column"))
                    {
                        cell = sheet.GetRow(InitialField_Row - 1).GetCell(GetColIndex("D"));
                        cell.SetCellValue(OnlyprogramName);

                    }
                    else
                    {
                        cell = sheet.GetRow(InitialField_Row - 1).GetCell(GetColIndex("C"));
                        cell.SetCellValue(OnlyprogramName);
                    }
                }
        ...
        private bool Writing_to_Report(bool WriteAgain = false, bool Fasttransfer = false)
        {
            try
            {
                if (WriteAgain == true && Fasttransfer == false)
                {
                    if (IncludeTolerence)
                    {
                        string ToleranceFileTemplate = DatabaseConstants.TemplatePath + @"\Tolerance.xls";
                        using (FileStream ToleranceStream = new FileStream(ToleranceFileTemplate, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                        {
                            toleranceWorkbook = new HSSFWorkbook(ToleranceStream);
                            toleranceSheet = toleranceWorkbook.GetSheet("Report");

                            using (FileStream ToleranceStream1 = new FileStream(ToleranceFileTemplate, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                            {
                                var Sheet = toleranceWorkbook.GetSheet("Report");

                                toleranceCell = Sheet.GetRow(8).GetCell(0);
                                toleranceCellStyleBorder = toleranceCell.CellStyle;
                                toleranceCell = Sheet.GetRow(7).GetCell(1);
                                toleranceCellStyleBorderRow = toleranceCell.CellStyle;
                                toleranceCell = Sheet.GetRow(9).GetCell(2);
                                toleranceCellStyleGreen = toleranceCell.CellStyle;
                                toleranceCell = Sheet.GetRow(10).GetCell(2);
                                toleranceCellStyleRed = toleranceCell.CellStyle;
                                toleranceWorkbook.Write(ToleranceStream1);
                                toleranceWorkbook.Close();
                                ToleranceStream1.Close();

                            }
                            toleranceWorkbook.Close();
                            ToleranceStream.Close();
                        }
                    }
                    int P_index = ReportName.LastIndexOf("_");
                    string ProgramPath;
                    if (P_index != -1 && ReportName.Substring(P_index + 7, 1) == "1")
                    {
                        ProgramPath = ReportName.Substring(0, P_index) + "_Report1" + ".xls";
                    }
                    else
                    {
                        ProgramPath = ReportName;
                    }
                    //string ProgramPath = ReportName + "_Report1" + ".xls";
                    using (FileStream stream3 = new FileStream(ProgramPath, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                    {
                        workbook = new HSSFWorkbook(stream3);
                        sheet = workbook.GetSheet("Report");

                        using (FileStream stream4 = new FileStream(ProgramPath, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                        {
                            int srNo = 0;
                            bool NewReportFirstTimeForMultiPP = false;
                            bool InsertedNewRowinExcelSheet = false;
                            bool InsertedNewColumninExcelSheet = false;
                            string tempRowValue = "temp";
                            string tempColValue = "temp";

                            var Sheet = workbook.GetSheet("Report");
                            for (var n = 7; n <= 200; n += 1)
                            {
                                var row = Sheet.GetRow(n);
                                try
                                {
                                    tempRowValue = row.GetCell(0).StringCellValue.ToString();
                                    if (tempRowValue != "" || tempRowValue != null)
                                    {
                                        if (ReportType.Contains("Column"))
                                        {
                                            if (tempRowValue == "" && n % 42 == 35 /*|| (tempRowValue == "" && n == 77) || (tempRowValue == "" && n == 119) || (tempRowValue == "" && n == 161) || (tempRowValue == "" && n == 203)*/)
                                            {
                                                n = n + 6;
                                                continue;
                                            }
                                            else if ((tempRowValue == "" && n > 43 && n < 49) || (tempRowValue == "" && n > 85 && n < 91) || (tempRowValue == "" && n > 127 && n < 133) || (tempRowValue == "" && n > 169 && n < 175) || (tempRowValue == "" && n > 211 && n < 217) || (tempRowValue == "" && n > 253 && n < 259))
                                            {
                                                continue;
                                            }
                                        }
                                        ...
                                        {
                                            if (tempRowValue == "" && n % 39 == 31)
                                            {
                                                n = n + 7;
                                                continue;
                                            }
                                            else if ((tempRowValue == "" && n > 40 && n < 46) || (tempRowValue == "" && n > 79 && n < 85) || (tempRowValue == "" && n > 118 && n < 124) || (tempRowValue == "" && n > 157 && n < 163) || (tempRowValue == "" && n > 196 && n < 202) || (tempRowValue == "" && n > 235 && n < 241) || (tempRowValue == "" && n > 274 && n < 280))
                                            {
                                                continue;
                                            }
                                        }
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    tempRowValue = row.GetCell(0).NumericCellValue.ToString();
                                    if (tempRowValue == "")
                                    {
                                        if (ReportType.Contains("Column"))
                                        {
                            ...
                            var rowCheck = Sheet.GetRow(7);
                            for (var j = 0; j <= 1000; j++)
                            {
                                try
                                {
                                    tempColValue = rowCheck.GetCell(j).StringCellValue.ToString();
                                    if (tempColValue == "")
                                    {
                                        if (ReportType.Contains("Column"))
                                        {
                                            LastColumnUsed = GetColumnName(j - 1);
                                            break;
                                        }
                                        else
                                        {
                                ...
                                catch (Exception ex)
                                {
                                    if(j!=0)
                                    {
                                        if (rowCheck.GetCell(j) == null || j % 14 == 0)
                                        {
                                            LastColumnUsed = GetColumnName(j - 3);
                                            break;
                                        }
                                    }
                                    tempColValue = rowCheck.GetCell(j).NumericCellValue.ToString();
                                    if (tempColValue == "")
                                    {
                                        if (ReportType.Contains("Column"))
                                        {
                            ...
                            if (ReportType.Contains("Column"))
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        cell = sheet.GetRow(i).GetCell(GetColIndex(InitialField_Column));
                                        if (cell.StringCellValue is null)
                                        {
                                            continue;
                                        }
                                        ...
                                        if (_IsDoPagination)
                                        {
                                            if (i % 42 == 30)
                                            {
                                                RowAfterWhich_PaginationShouldBeDone = i;
                                                rowNo = i+12;
                                            }
                                            if (i == RowAfterWhich_PaginationShouldBeDone) // MaximumFilledRowPerTemplate Then
                                            {
                                                ...
                                                {
                                                    for (int colCount = 0; colCount < MeasurementStringList.Count; colCount += 13)
                                                    {
                                                        colNoPag = colCount + WriteAgainColIncrement;
                                                        var reportSheet = workbook.GetSheet("Report");
                                                        var dataSheet = workbook.GetSheet("Data");
                                                        string tempRange = SourceInitialRange + ":" + "N42";
                                                        var Range = tempRange;
                                                        var cellRange = CellRangeAddress.ValueOf(Range);

                                                        if(WriteAgainColIncrement == 0)
                                                        {
                                                            for (var n = cellRange.FirstRow; n <= cellRange.LastRow; n++)
                                                            {
                                                                var row = dataSheet.GetRow(n);
                                                                IRow secondRow = reportSheet.CreateRow(n + rowNo);
                                                            }
                                                        }
                                                        ...
                                                        {
                                                            var row = dataSheet.GetRow(n);
                                                            IRow secondRow = reportSheet.GetRow(n + rowNo);
                                                            for (var j = cellRange.FirstColumn; j <= cellRange.LastColumn; j++)
                                                            {
                                                                var tempValue = row.GetCell(j).CellStyle;
                                                                secondRow.CreateCell(j+ colNoPag).CellStyle = tempValue;
                                                            }
                                                        }

                                                        for (int jj = 7; jj < 34; jj++)
                                                        {
                                                            IRow secondRow5 = reportSheet.GetRow(jj);
                                                            var temptesteStyle = secondRow5.GetCell(1).CellStyle;
                                                            IRow secondRow51 = reportSheet.GetRow(jj + rowNo);
                                                            secondRow51.GetCell(1+ colNoPag).CellStyle = temptesteStyle;
                                                            var temptesteStyle1 = secondRow5.GetCell(0).CellStyle;
                                                            secondRow51.GetCell(0+ colNoPag).CellStyle = temptesteStyle1;
                                                        }

                                                        var cra = new NPOI.SS.Util.CellRangeAddress(rowNo, rowNo + 4, colNoPag, colNoPag+13);
                                                        reportSheet.AddMergedRegion(cra);
                                                        IRow Row1 = reportSheet.GetRow(0);
                                                        var tempStyle = Row1.GetCell(0).CellStyle;
                                                        IRow Row11 = reportSheet.GetRow(rowNo + 1);
                                                        Row11.GetCell(0 + colNoPag).CellStyle = tempStyle;
                                                        string CompanyName = Row1.GetCell(0).StringCellValue;
                                                        Row11.GetCell(0 + colNoPag).SetCellValue(CompanyName);

                                                        var cra1 = new NPOI.SS.Util.CellRangeAddress(rowNo + 34, rowNo + 36, colNoPag, colNoPag+13);
                                                        reportSheet.AddMergedRegion(cra1);
                                                        IRow Row2 = reportSheet.GetRow(34);
                                                        var tempStyle1 = Row2.GetCell(0).CellStyle;
                                                        IRow Row21 = reportSheet.GetRow(rowNo + 34);
                                                        Row21.GetCell(0 + colNoPag).CellStyle = tempStyle1;
                                                        string Notes = Row2.GetCell(0).StringCellValue;
                                                        Row21.GetCell(0 + colNoPag).SetCellValue(Notes);


                                                        var cra2 = new NPOI.SS.Util.CellRangeAddress(rowNo + 37, rowNo + 41, colNoPag, colNoPag+4);
                                                        reportSheet.AddMergedRegion(cra2);
                                                        IRow Row3 = reportSheet.GetRow(37);
                                                        var tempStyle2 = Row3.GetCell(0).CellStyle;
                                                        IRow Row31 = reportSheet.GetRow(rowNo + 37);
                                                        Row31.GetCell(0+ colNoPag).CellStyle = tempStyle2;
                                                        string Checked_By = Row3.GetCell(0).StringCellValue;
                                                        Row31.GetCell(0+ colNoPag).SetCellValue(Checked_By);

                                                        var cra3 = new NPOI.SS.Util.CellRangeAddress(rowNo + 37, rowNo + 41, colNoPag+5, colNoPag+8);
                                                        reportSheet.AddMergedRegion(cra3);
                                                        IRow Row4 = reportSheet.GetRow(37);
                                                        var tempStyle3 = Row4.GetCell(4).CellStyle;
                                                        IRow Row41 = reportSheet.GetRow(rowNo + 37);
                                                        Row41.GetCell(5 + colNoPag).CellStyle = tempStyle3;
                                                        string Verified_By = Row4.GetCell(4).StringCellValue;
                                                        Row41.GetCell(5 + colNoPag).SetCellValue(Verified_By);

                                                        var cra4 = new NPOI.SS.Util.CellRangeAddress(rowNo + 37, rowNo + 41, colNoPag+9, colNoPag+13);
                                                        reportSheet.AddMergedRegion(cra4);
                                                        IRow Row5 = reportSheet.GetRow(37);
                                                        var tempStyle4 = Row5.GetCell(9).CellStyle;
                                                        IRow Row51 = reportSheet.GetRow(rowNo + 37);
                                                        Row51.GetCell(9 + colNoPag).CellStyle = tempStyle4;
                                                        string Approved_By = Row5.GetCell(9).StringCellValue;
                                                        Row51.GetCell(9 + colNoPag).SetCellValue(Approved_By);

                                                        IRow secondRow2 = reportSheet.GetRow(0);
                                                        string cellValue = secondRow2.GetCell(0).StringCellValue;
                                                        IRow secondRow21 = reportSheet.GetRow(rowNo);
                                                        secondRow21.GetCell(0 + colNoPag).SetCellValue(cellValue);

                                                        for (int ii = rowNo + 30; ii < rowNo + 34; ii++)
                                                        {
                                                            var row4 = dataSheet.GetRow(30);
                                                            IRow secondRow4 = reportSheet.GetRow(ii);
                                                            var tempValue4 = row4.GetCell(0).StringCellValue;
                                                            secondRow4.GetCell(0 + colNoPag).SetCellValue(tempValue4);
                                                        }
                                                        WriteAgainColIncrement++;
                                                    }
                                                    WriteAgainColIncrement = 0;
                                                }
                                                ...
                                                cell = sheet.GetRow(i - 1).GetCell(GetColIndex(InitialField_Column) - 1);
                                                try
                                                {
                                                    srNo = Conversions.ToInteger(cell.StringCellValue);
                                                }
                                                catch (Exception ex)
                                                {
                                                    srNo = Conversions.ToInteger(cell.NumericCellValue);
                                                }
                                                break;
                                            }
                                            else
                                            {
                                                cell = sheet.GetRow(i).GetCell(GetColIndex(InitialField_Column));
                                                if (Conversions.ToBoolean(Operators.ConditionalCompareObjectNotEqual(cell.StringCellValue, null, false)))
                                                {
                                                    try
                                                    {
                                                        LastRowUsed = i + 2;
                                        ...
                                        {
                                            cell = sheet.GetRow(i).GetCell(GetColIndex(InitialField_Column));
                                            string ss = cell.StringCellValue;
                                            string ssNext;
                                            try
                                            {
                                                cell = sheet.GetRow(i + 1).GetCell(GetColIndex(InitialField_Column));
                                                ssNext = cell.StringCellValue;
                                            }
                                            catch (Exception exx)
                                            {
                                                ssNext = null;
                                            ...
                                            if (ss is object)
                                            {
                                                ...
                                                if (ssNext is object)
                                                {
                                                    ...
                                                    {
                                                        int ssNo;
                                                        cell = sheet.GetRow(LastRowUsed - 1).GetCell(GetColIndex(InitialField_Column));
                                                        if (int.TryParse(Conversions.ToString(cell.StringCellValue), out ssNo))
                                                        {
                                                            srNo = ssNo;
                                                        }
                                                        else
                            ...
                            else
                            {
                                ...
                                {
                                    cell = sheet.GetRow(InitialField_Row).GetCell(i);
                                    if (cell.StringCellValue is null)
                                    {
                                        continue;
                                    }
                                    if (i % 14 == 10)
                                    {
                                        ColumnAfterWhich_PaginationShouldBeDone = GetColumnName(i);
                                        colNo = i + 4;
                                    }
                                    ...
                                    if (_IsDoPagination)
                                    {
                                        if ((GetColumnName(i) ?? "") == (ColumnAfterWhich_PaginationShouldBeDone ?? "")) // ' MaximumFilledColumnPerTemplate Then
                                        {
                                            ...
                                            {
                                                for (int rowCount = 0; rowCount < MeasurementStringList.Count; rowCount += 22)
                                                {
                                                    rowNoPag = rowCount + WriteAgainRowIncrement;
                                                    var reportSheet = workbook.GetSheet("Report");
                                                    var dataSheet = workbook.GetSheet("Data");
                                                    string tempRange = SourceInitialRange + ":" + "N39";
                                                    var Range = tempRange;
                                                    var cellRange = CellRangeAddress.ValueOf(Range);
                                                    ...
                                                    {
                                                        var row = dataSheet.GetRow(n);
                                                        IRow secondRow = reportSheet.GetRow(n + rowNoPag);
                                                        for (var j = cellRange.FirstColumn; j <= cellRange.LastColumn; j++)
                                                        {
                                                            var tempValue = row.GetCell(j).CellStyle;
                                                            secondRow.CreateCell(j + colNo).CellStyle = tempValue;
                                                        }
                                                    }
                                                    for (int jj = 7; jj < 30; jj++)
                                                    {
                                                        IRow secondRow5 = reportSheet.GetRow(jj + rowNoPag);
                                                        var temptesteStyle = secondRow5.GetCell(1).CellStyle;
                                                        secondRow5.GetCell(colNo).CellStyle = temptesteStyle;
                                                        var temptesteStyle1 = secondRow5.GetCell(0).CellStyle;
                                                        secondRow5.GetCell(colNo).CellStyle = temptesteStyle1;
                                                    }

                                                    var cra = new NPOI.SS.Util.CellRangeAddress(rowNoPag, rowNoPag + 4, colNo, colNo + 13);
                                                    reportSheet.AddMergedRegion(cra);
                                                    IRow Row1 = reportSheet.GetRow(rowNoPag);
                                                    var tempStyle = Row1.GetCell(0).CellStyle;
                                                    Row1.GetCell(colNo).CellStyle = tempStyle;
                                                    string CompanyName = Row1.GetCell(0).StringCellValue;
                                                    Row1.GetCell(colNo).SetCellValue(CompanyName);

                                                    var cra1 = new NPOI.SS.Util.CellRangeAddress(rowNoPag + 30, rowNoPag + 34, colNo, colNo + 13);
                                                    reportSheet.AddMergedRegion(cra1);
                                                    IRow Row2 = reportSheet.GetRow(rowNoPag + 30);
                                                    var tempStyle1 = Row2.GetCell(0).CellStyle;
                                                    Row2.GetCell(colNo).CellStyle = tempStyle1;
                                                    string Notes = Row2.GetCell(0).StringCellValue;
                                                    Row2.GetCell(colNo).SetCellValue(Notes);


                                                    var cra2 = new NPOI.SS.Util.CellRangeAddress(rowNoPag + 35, rowNoPag + 38, colNo, colNo + 3);
                                                    reportSheet.AddMergedRegion(cra2);
                                                    IRow Row3 = reportSheet.GetRow(rowNoPag + 35);
                                                    var tempStyle2 = Row3.GetCell(0).CellStyle;
                                                    Row3.GetCell(colNo).CellStyle = tempStyle2;
                                                    string Checked_By = Row3.GetCell(0).StringCellValue;
                                                    Row3.GetCell(colNo).SetCellValue(Checked_By);

                                                    var cra3 = new NPOI.SS.Util.CellRangeAddress(rowNoPag + 35, rowNoPag + 38, colNo + 4, colNo + 8);
                                                    reportSheet.AddMergedRegion(cra3);
                                                    IRow Row4 = reportSheet.GetRow(rowNoPag + 35);
                                                    var tempStyle3 = Row4.GetCell(4).CellStyle;
                                                    Row4.GetCell(colNo + 4).CellStyle = tempStyle3;
                                                    string Verified_By = Row4.GetCell(4).StringCellValue;
                                                    Row4.GetCell(colNo + 4).SetCellValue(Verified_By);

                                                    var cra4 = new NPOI.SS.Util.CellRangeAddress(rowNoPag + 35, rowNoPag + 38, colNo + 9, colNo + 13);
                                                    reportSheet.AddMergedRegion(cra4);
                                                    IRow Row5 = reportSheet.GetRow(rowNoPag + 35);
                                                    var tempStyle4 = Row5.GetCell(9).CellStyle;
                                                    Row5.GetCell(colNo + 9).CellStyle = tempStyle4;
                                                    string Approved_By = Row5.GetCell(9).StringCellValue;
                                                    Row5.GetCell(colNo + 9).SetCellValue(Approved_By);

                                                    IRow secondRow2 = reportSheet.GetRow(rowNoPag);
                                                    string cellValue = secondRow2.GetCell(0).StringCellValue;
                                                    secondRow2.GetCell(colNo).SetCellValue(cellValue);

                                                    for (int ii = 10; ii < 14; ii++)
                                                    {
                                                        var row4 = dataSheet.GetRow(7);
                                                        IRow secondRow4 = reportSheet.GetRow(7 + rowNoPag);
                                                        var tempValue4 = row4.GetCell(ii).StringCellValue;
                                                        secondRow4.GetCell(ii + colNo).SetCellValue(tempValue4);
                                                    }
                                                    WriteAgainRowIncrement += 17;
                                                }
                                                WriteAgainRowIncrement = 0;
                                            }
                                            ...
                                            try
                                            {
                                                cell = sheet.GetRow(Currentrow - 1).GetCell(GetColIndex(LastColumnUsed) - 7);
                                                srNo = (int)cell.NumericCellValue;
                                            }
                                            catch(Exception ex)
                                            {
                                                cell = sheet.GetRow(Currentrow - 1).GetCell(GetColIndex(LastColumnUsed) - 7);
                                                if(cell.StringCellValue=="Mean")
                                                {
                                                    cell = sheet.GetRow(Currentrow - 1).GetCell(GetColIndex(LastColumnUsed) - 8);
                                                    srNo = (int)cell.NumericCellValue;
                                                }
                                                else if(cell.StringCellValue == "Min")
                                                {
                                                    cell = sheet.GetRow(Currentrow - 1).GetCell(GetColIndex(LastColumnUsed) - 10);
                                                    srNo = (int)cell.NumericCellValue;
                                                }
                                            }
                                            cell = sheet.GetRow(Currentrow - 1).GetCell(GetColIndex(LastColumnUsed) - 1);
                                            cell.SetCellValue((double)srNo + 1);
                                            ...
                                            for (int j = 0, loopTo4 = Fieldrows.Length - 1; j <= loopTo4; j++)
                                            {
                                                ...
                                                if ((Currentrow % 39) == 31)
                                                {
                                                    cell = sheet.GetRow(Currentrow + 15).GetCell(GetColIndex(LastColumnUsed) - 1);
                                                    cell.SetCellValue(srNo + 1);
                                                    Currentrow = Currentrow + 17;
                                                }
                                                if (!string.IsNullOrEmpty(Fieldrows[j][13].ToString()))
                                                {
                                                    for (int ii = 0, loopTo5 = MeasurementStringList.Count - 1; ii <= loopTo5; ii++)
                                                    {
                                                    //for (int k = 0; k <= Fieldrows.Length; k++)
                                                    //{
                                                    //    if ((Fieldrows[k][13].ToString() ?? "") != (MeasurementStringList[ii][6].ToString() ?? ""))
                                                    //        continue;
                                                    //    if ((Fieldrows[k][13].ToString() ?? "") == (MeasurementStringList[ii][6].ToString() ?? ""))
                                                    //        break;
                                                    //}
                                                        if ((Fieldrows[j][13].ToString() ?? "") != (MeasurementStringList[ii][6].ToString() ?? ""))
                                                            continue;
                                                        if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(MeasurementStringList[ii][0], "Date", false)))
                                                        {
                                                            CompareAndWrite_FieldValues(Conversions.ToString(MeasurementStringList[ii][2]), Currentrow, GetColumnName(GetColIndex(LastColumnUsed)), NumberFormat: -2);
                                                        }
                                                        else if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(MeasurementStringList[ii][0], "Time", false)))
                                                        {
                                                            CompareAndWrite_FieldValues(Conversions.ToString(MeasurementStringList[ii][2]), Currentrow, GetColumnName(GetColIndex(LastColumnUsed)), NumberFormat: -3);
                                                        }
                                                        else
                                                        {
                                                            if (IncludeTolerence)
                                                            {
                                                                try
                                                                {
                                                                    cell = sheet.GetRow(Currentrow - 1).GetCell(1);
                                                                    var valueMinAndMax = cell.StringCellValue;
                                                                    minimumValue[i] = double.Parse(valueMinAndMax.Substring(0, valueMinAndMax.LastIndexOf('-') - 2));
                                                                    maximumValue[i] = double.Parse(valueMinAndMax.Substring(valueMinAndMax.LastIndexOf('-') + 2));
                                                                }
                                                                catch (Exception ex)
                                                                {
                                                                    cell = sheet.GetRow(Currentrow - 1).GetCell(2);
                                                                    minimumValue[i] = cell.NumericCellValue;
                                                                    cell = sheet.GetRow(Currentrow - 1).GetCell(3);
                                                                    maximumValue[i] = cell.NumericCellValue;
                                                                }
                                                            }
                                                            count = i;
                                                            CompareAndWrite_FieldValues(Conversions.ToString(MeasurementStringList[ii][2]), Currentrow, GetColumnName(GetColIndex(LastColumnUsed)), NumberFormat: Formatdigits);
                                                        }
                                                        ...
                                                        if (Fieldrows[j][5].ToString().Contains(Conversions.ToString(degree)) & ToleranceFormat != 0)
                                                        {
                                                            ...
                                                            if (!LsAbsent && !HsAbsent)
                                                            {
                                                                ...
                                                                {
                                                                    cell = sheet.GetRow(Currentrow).GetCell(GetColIndex(LastColumnUsed));
                                                                    testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                                                                ...
                                                                {
                                                                    cell = sheet.GetRow(Currentrow).GetCell(GetColIndex(LastColumnUsed));
                                                                    testeStyle.FillForegroundColor = IndexedColors.Orange.Index;
                                                                    testeStyle.FillPattern = FillPattern.SolidForeground;
                                                                    cell.CellStyle = testeStyle;
                                                        ...
                                                        if (ShowAvging && !_IsDoPagination) // 'Update Formulas for statistics if we have inserted a new row for entering values.
                                                        {
                                                            if (InsertedNewRowinExcelSheet && !Fieldrows[j][5].ToString().Contains(Conversions.ToString(degree)))
                                                            {
                                                                ...
                                                                for (int iI = 1; iI <= 3; iI++)
                                                                {
                                                                    ...
                                                                    cell = sheet.GetRow(Currentrow).GetCell(GetColIndex(formularCol));
                                                                    formula_String = cell.CellFormula.ToString();
                                                                    if (!string.IsNullOrEmpty(formula_String))
                                                                    {
                                                                        ...
                                                                        cell = sheet.GetRow(LastRowUsed + ii).GetCell(GetColIndex(formularCol));
                                                                        cell.SetCellFormula(formula_String);
                                                ...
                                                //    cell = sheet.CreateRow(Currentrow).CreateCell(GetColIndex(LastColumnUsed));
                                                //    cell.SetCellValue((double)srNo + 1);
                                            ...
                                            //cell = sheet.GetRow(InitialField_Row-1).GetCell(i-1);
                                            //srNo = Conversions.ToInteger(cell.NumericCellValue);
                                        ...
                                        else
                                        {
                                            ...
                                            cell = sheet.GetRow(InitialField_Row).GetCell(i - 1);
                                            try
                                            {
                                                if (cell.StringCellValue == "")
                                                {
                                                    LastColumnUsed = GetColumnName(i);
                                                }
                                            }
                                            catch (Exception ex)
                                            {
                                    ...
                                    else
                                    {
                                        ...
                                        cell = sheet.GetRow(InitialField_Row).GetCell(GetColIndex(str));
                                        string ss = Conversions.ToString(cell.StringCellValue);
                                        string ssNext;
                                        try
                                        {
                                            cell = sheet.GetRow(InitialField_Row).GetCell(i + 1);
                                            ssNext = cell.StringCellValue; // ', InitialField_Column).ToString
                                        }
                                        catch (Exception exx)
                                        {
                                            ssNext = null;
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                cell = sheet.GetRow(InitialField_Row).GetCell(i - 1);
                                                srNo = Conversions.ToInteger(cell.StringCellValue); // GetColIndex(str)
                                            }
                                            catch (InvalidCastException ex)
                                            {
                                                srNo = 0;
                            ...
                            if (IsGrigImagecaptureM)
                            {
                                ...
                                for (int i = 0, loopTo3 = MeasurementStringList.Count - 1; i <= loopTo3; i++)
                                {
                                    ...
                                    if (FieldInt == 1)
                                    {
                                        cell = sheet.GetRow(LastRowUsed).GetCell(a);
                                        testeStyle.FillForegroundColor = IndexedColors.Green.Index;
                                        testeStyle.FillPattern = FillPattern.SolidForeground;
                                        cell.CellStyle = testeStyle;
                                    }
                                    else if (FieldInt == 0)
                            ...
                            else if (IsHCM)
                            {
                                ...
                                if (HCMno > 0)
                                {
                                    ...
                                    {
                                        cell = sheet.GetRow(2).GetCell(ii);
                                        var vall = cell.StringCellValue;
                            ...
                            else if (ReportType.Contains("Column"))
                            {
                                ...
                                if (LastRowUsed == 9)
                                {
                                    cell = sheet.GetRow(LastRowUsed - 1).GetCell(CurrentUsedColumn - 1);
                                    cell.SetCellValue(srNo + 1);
                                }
                                else if ((LastRowUsed % 42) == 9)
                                {
                                    cell = sheet.GetRow(LastRowUsed - 1).GetCell(CurrentUsedColumn - 1);
                                    cell.SetCellValue(srNo + 1);
                                }
                                else
                                {
                                    try
                                    {
                                        cell = sheet.GetRow(LastRowUsed - 2).GetCell(CurrentUsedColumn - 1);
                                        srNo = (int)cell.NumericCellValue;
                                    ...
                                    {
                                        cell = sheet.GetRow(LastRowUsed - 2).GetCell(CurrentUsedColumn - 1);
                                        if(cell.StringCellValue == "Allowed Range" || cell.StringCellValue == "± Tol.")
                                        {
                                            cell = sheet.GetRow(LastRowUsed - 1).GetCell(CurrentUsedColumn - 1);
                                            cell.SetCellValue(srNo + 1);
                                        }
                                        else if(cell.StringCellValue == "USL")
                                        {
                                            cell = sheet.GetRow(LastRowUsed - 1).GetCell(CurrentUsedColumn - 1);
                                            cell.SetCellValue(srNo + 1);
                                        }
                                ...
                                testeStyle = workbook.CreateCellStyle();
                                testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                                            ...
                                            for (int i = 0, loopTo8 = MeasurementStringList.Count - 1; i <= loopTo8; i++)
                                            {
                                                ...
                                                for (int k = 0; k <= Fieldrows.Length; k++)
                                                {
                                                    if ((Fieldrows[j][13].ToString() ?? "") != (MeasurementStringList[i][6].ToString() ?? ""))
                                                        continue;
                                                    if ((Fieldrows[k][13].ToString() ?? "") == (MeasurementStringList[i][6].ToString() ?? ""))
                                                        break;
                                                }
                                                if ((CurrentUsedColumn % firstColumnInNewPage) == 1)
                                                {
                                                    cell = sheet.GetRow(LastRowUsed - 1).GetCell(CurrentUsedColumn - 1);
                                                    cell.SetCellValue(srNo + 1);
                                                    CurrentUsedColumn = CurrentUsedColumn + 1;
                                                }
                                                ...
                                                else
                                                {
                                                    if(IncludeTolerence)
                                                    {
                                                        try
                                                        {
                                                            cell = sheet.GetRow(8).GetCell(CurrentUsedColumn - 1);
                                                            var valueMinAndMax = cell.StringCellValue;
                                                            int length = valueMinAndMax.Length;
                                                            if (valueMinAndMax.Contains("+"))
                                                            {
                                                                if(valueMinAndMax.Contains("°"))
                                                                {
                                                                    string tempAngleValue = valueMinAndMax.Replace("°", "");
                                                                    string tempAngleValue1 = tempAngleValue.Replace("'", "");
                                                                    string tempAngleValue2 = tempAngleValue1.Replace("\"", "");
                                                                    string tempAngleValue3 = tempAngleValue2.Replace(" ", "");
                                                                    string tempAngleValue4 = tempAngleValue3.Replace("-", " -");
                                                                    string angleValue = tempAngleValue4.Replace("+", " +");
                                                                    if (angleValue.Contains("+"))
                                                                    {

                                                                        minimumValue[i] = double.Parse(angleValue.Substring(0, angleValue.LastIndexOf('+') - 1)) - double.Parse(angleValue.Substring(angleValue.LastIndexOf('-') + 1));
                                                                        maximumValue[i] = double.Parse(angleValue.Substring(0, angleValue.IndexOf('+') - 1)) + double.Parse(angleValue.Substring(angleValue.IndexOf('+') + 1, angleValue.Length - angleValue.IndexOf('-')));
                                                                    }
                                                                    else
                                                                    {
                                                                        minimumValue[i] = double.Parse(angleValue.Substring(0, angleValue.LastIndexOf('-') - 2));
                                                                        maximumValue[i] = double.Parse(angleValue.Substring(angleValue.LastIndexOf('-') + 2));
                                                                    }

                                                                }
                                                                else
                                                                {
                                                                    minimumValue[i] = double.Parse(valueMinAndMax.Substring(0, valueMinAndMax.LastIndexOf('+') - 1)) - double.Parse(valueMinAndMax.Substring(valueMinAndMax.LastIndexOf('-') + 1));
                                                                    maximumValue[i] = double.Parse(valueMinAndMax.Substring(0, valueMinAndMax.IndexOf('+') - 1)) + double.Parse(valueMinAndMax.Substring(valueMinAndMax.IndexOf('+') + 1, valueMinAndMax.Length - valueMinAndMax.IndexOf('-')));
                                                                }
                                                            }
                                                            else
                                                            {
                                                        ...
                                                        {
                                                            cell = sheet.GetRow(9).GetCell(CurrentUsedColumn - 1);
                                                            minimumValue[i] = cell.NumericCellValue;
                                                            cell = sheet.GetRow(10).GetCell(CurrentUsedColumn - 1);
                                                            maximumValue[i] = cell.NumericCellValue;
                                                        }
                                                ...
                                                if (Fieldrows[j][5].ToString().Contains(Conversions.ToString(degree)) && ToleranceFormat != 0)
                                                {
                                                    ...
                                                    if (!LsAbsent && !HsAbsent)
                                                    {
                                                        if (ConvertDegMinSecToDecimal(Conversions.ToString(MeasurementStringList[i][2])) <= ConvertDegMinSecToDecimal(Conversions.ToString(Fieldrows[j][11])) & ConvertDegMinSecToDecimal(Conversions.ToString(MeasurementStringList[i][2])) >= ConvertDegMinSecToDecimal(Conversions.ToString(Fieldrows[j][12])))
                                                        {
                                                            cell = sheet.GetRow(LastRowUsed).GetCell(CurrentUsedColumn);
                                                            testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                                                            testeStyle.FillPattern = FillPattern.SolidForeground;
                                                            cell.CellStyle = testeStyle;
                                                            // 35 means green colour
                                                        }
                                                        else
                                                        {
                                                            cell = sheet.GetRow(LastRowUsed).GetCell(CurrentUsedColumn);
                                                ...
                                                if (ShowAvging && !_IsDoPagination) // 'Update Formulas for statistics if we have inserted a new row for entering values.
                                                {
                                                    if (InsertedNewRowinExcelSheet && !Fieldrows[j][5].ToString().Contains(Conversions.ToString(degree)))
                                                    {
                                                        ...
                                                        for (int ii = 1; ii <= 3; ii++)
                                                        {
                                                            cell = sheet.GetRow(LastRowUsed + ii).GetCell(GetColIndex(currColumn));
                                                            formula_String = cell.CellFormula.ToString();
                                                            if (!string.IsNullOrEmpty(formula_String))
                                                            {
                            ...
                            else if (ReportType.Contains("TVS"))                          // for delphiTVS report measurement filling 16/04/2014
                            {
                                ...
                                if (passStatus == PassFail.pass)
                                {
                                    ...
                                    cell = sheet.GetRow(LastRowUsed).GetCell(GetColIndex("E"));
                                    testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                                    testeStyle.FillPattern = FillPattern.SolidForeground;
                                    cell.CellStyle = testeStyle;
                            ...
                            else
                            {
                                ...
                                if (tempRowPagination != 1)
                                {
                                    int Currentrow = InitialField_Row;
                                    cell = sheet.GetRow(Currentrow - 1).GetCell(GetColIndex(LastColumnUsed) - 2);
                                    if (Currentrow == 8 && (GetColIndex(LastColumnUsed) - 2) == 0)
                                    {
                                        srNo = 0;
                                    }
                                    else
                            ...
                            if (IsDTVSM)     // for delphi tvs
                            {
                                try
                                {
                                    ...
                                    // correction by Vinod Jakhar... here Name is repplaced by "ImageSheet" 
                                    if (!WriteAgain)
                                    {
                ...
                else
                {
                    ...
                    {
                        string ToleranceFileTemplate = DatabaseConstants.TemplatePath + @"\Tolerance.xls";
                        using (FileStream ToleranceStream = new FileStream(ToleranceFileTemplate, FileMode.Open, FileAccess.Read))
                        {
                            toleranceWorkbook = new HSSFWorkbook(ToleranceStream);
                            toleranceSheet = toleranceWorkbook.GetSheet("Report");
                    ...
                    {
                        DestinationFile = ReportName;
                        int P_index = DestinationFile.LastIndexOf(@"\");
                        int P_index2 = DestinationFile.LastIndexOf(".");
                        tmpStr = DestinationFile.Substring(P_index + 1, P_index2 - P_index - 1);
                    }
                    else
                    {
                        int P_index = ProgramName.LastIndexOf(@"\");
                        int P_index2 = ProgramName.LastIndexOf(".");
                        string ProgramsPath = ProgramName.Substring(0, P_index);
                        int P_index3 = ProgramsPath.LastIndexOf(@"\");
                        tmpStr = ProgramName.Substring(P_index + 1, P_index2 - P_index - 1);
                        DestinationFile = ProgramsPath.Substring(0, P_index3) + "\\" + "Reports" + "\\" + tmpStr + "_Report1" + ".xls";
                    }

                    int pN_L_row, pN_L_col;
                    System.IO.FileInfo ReportTemplateFile = new System.IO.FileInfo(ReportTemplateName);
                    if (ReportTemplateFile.Exists)
                    {
                        System.IO.FileInfo DestinationFileStatus = new System.IO.FileInfo(DestinationFile);
                        if (DestinationFileStatus.Exists)
                        {
                            DestinationFileStatus.Delete();
                        }
                        ReportTemplateFile.CopyTo(DestinationFile);
                    }

                    using (FileStream stream1 = new FileStream(DestinationFile, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                    {
                        workbook = new HSSFWorkbook(stream1);
                        sheet = workbook.GetSheet("Report");

                        using (FileStream stream2 = new FileStream(DestinationFile, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                        {
                            pN_L_row = InitialField_Row - 1 + PartNameLabelLocation_Row;
                            pN_L_col = (GetColIndex(InitialField_Column) - 1);


                            cell = sheet.GetRow(pN_L_row).GetCell(pN_L_col);
                            cell.SetCellValue("Part:" + tmpStr);
                            testeStyle = workbook.CreateCellStyle();
                            testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                            ...
                            if (!WriteAgain)
                            {
                                    ...
                                    if (!Fasttransfer)
                                    {
                                        ...
                                        if (ReportType.Contains("Column"))
                                        {
                                            ...
                                            if (pgn > 0 && diff == 1)
                                            {
                                                ...
                                                {
                                                    var firstSheet = workbook.GetSheet("Report");
                                                    var secondSheet = workbook.GetSheet("Data");
                                                    string tempRange = SourceInitialRange + ":" + SourceFinalRange;
                                                    var Range = tempRange;
                                                    var cellRange = CellRangeAddress.ValueOf(Range);
                                        ...
                                        if (UseExistingReport)
                                        {
                                            if (!AllProgramInSingleReport)
                                            {
                                                ...
                                                //Worksheet xlDatasht;
                                                //xL.workbook.GetSheet(tmpStr + "Data");
                                            ...
                                            {
                                                var firstSheet = workbook.GetSheet(SelectedDataSheet);
                                                var secondSheet = workbook.GetSheet("Data");
                                                string tempRange = SourceInitialRange + ":" + SourceFinalRange;
                                                var Range = tempRange;
                                                var cellRange = CellRangeAddress.ValueOf(Range);
                                                ...
                                                sheet = workbook.GetSheet(SelectedDataSheet);
                                                var tempRange1 = SourceInitialRange + ":" + SourceFinalRange;
                                                var Range1 = tempRange;
                                                var cellRange1 = CellRangeAddress.ValueOf(Range);
                                                ...
                                                sheet = workbook.GetSheet(SelectedDataSheet);
                                                string tempRange2 = (InitialField_Column + InitialField_Row) + ":" + TemplateFinalRange;
                                                var Range2 = tempRange;
                                                var cellRange2 = CellRangeAddress.ValueOf(Range);
                                                for (var i = cellRange.FirstRow; i <= cellRange.LastRow; i++)
                                                {
                                                    var row = sheet.GetRow(i);
                                                    for (var j = cellRange.FirstColumn; j <= cellRange.LastColumn; j++)
                                ...
                                if (ReportType.Contains("Column"))
                                {
                                    for(int colCount=13; colCount < MeasurementStringList.Count; colCount+=13)
                                    {
                                        colNo = colCount + colIncrement;
                                        var reportSheet = workbook.GetSheet("Report");
                                        var dataSheet = workbook.GetSheet("Data");
                                        string tempRange = SourceInitialRange + ":" + "N42";
                                        var Range = tempRange;
                                        ...
                                        {
                                            var row = dataSheet.GetRow(n);
                                            IRow secondRow = reportSheet.GetRow(n);
                                            for (var j = cellRange.FirstColumn; j <= cellRange.LastColumn; j++)
                                            {
                                                var tempValue = row.GetCell(j).CellStyle;
                                                secondRow.CreateCell(j + colNo).CellStyle = tempValue;
                                        ...
                                        var cra = new NPOI.SS.Util.CellRangeAddress(0, 4, colNo, colNo + 13);
                                        reportSheet.AddMergedRegion(cra);
                                        IRow Row1 = reportSheet.GetRow(0);
                                        var tempStyle = Row1.GetCell(0).CellStyle;
                                        Row1.GetCell(colNo).CellStyle = tempStyle;
                                        ...
                                        var cra1 = new NPOI.SS.Util.CellRangeAddress(34, 36, colNo, colNo + 13);
                                        reportSheet.AddMergedRegion(cra1);
                                        IRow Row2 = reportSheet.GetRow(34);
                                        var tempStyle1 = Row2.GetCell(0).CellStyle;
                                        Row2.GetCell(colNo).CellStyle = tempStyle1;
                                        ...
                                        var cra2 = new NPOI.SS.Util.CellRangeAddress(37, 41, colNo, colNo + 4);
                                        reportSheet.AddMergedRegion(cra2);
                                        IRow Row3 = reportSheet.GetRow(37);
                                        var tempStyle2 = Row3.GetCell(0).CellStyle;
                                        Row3.GetCell(colNo).CellStyle = tempStyle2;
                                        ...
                                        var cra3 = new NPOI.SS.Util.CellRangeAddress(37, 41, colNo + 5, colNo + 8);
                                        reportSheet.AddMergedRegion(cra3);
                                        IRow Row4 = reportSheet.GetRow(37);
                                        var tempStyle3 = Row4.GetCell(4).CellStyle;
                                        Row4.GetCell(colNo + 5).CellStyle = tempStyle3;
                                        string Verified_By = Row4.GetCell(4).StringCellValue;
                                        Row4.GetCell(colNo + 5).SetCellValue(Verified_By);

                                        var cra4 = new NPOI.SS.Util.CellRangeAddress(37, 41, colNo + 9, colNo + 13);
                                        reportSheet.AddMergedRegion(cra4);
                                        IRow Row5 = reportSheet.GetRow(37);
                                        var tempStyle4 = Row5.GetCell(9).CellStyle;
                                        Row5.GetCell(colNo + 9).CellStyle = tempStyle4;
                                        ...
                                        IRow secondRow2 = reportSheet.GetRow(0);
                                        string cellValue = secondRow2.GetCell(0).StringCellValue;
                                        secondRow2.GetCell(14).SetCellValue(cellValue);

                                        for (int ii = 30; ii < 34; ii++)
                                        {
                                            var row4 = dataSheet.GetRow(ii);
                                            IRow secondRow4 = reportSheet.GetRow(ii);
                                            var tempValue4 = row4.GetCell(0).StringCellValue;
                                            secondRow4.GetCell(0 + colNo).SetCellValue(tempValue4);
                                        }
                                        colIncrement++;
                                    }
                                    colIncrement = 1;
                                ...
                                {
                                    for(int rowCount=22; rowCount < MeasurementStringList.Count; rowCount+=22)
                                    {
                                        rowNo = rowCount + rowIncrement;
                                        var reportSheet = workbook.GetSheet("Report");
                                        var dataSheet = workbook.GetSheet("Data");
                                        string tempRange = SourceInitialRange + ":" + "N39";
                                        var Range = tempRange;
                                        ...
                                        for (var n = cellRange.FirstRow; n <= cellRange.LastRow; n++)
                                        {
                                            ...
                                            {
                                                var tempValue = row.GetCell(j).CellStyle;
                                                secondRow.CreateCell(j).CellStyle = tempValue;
                                            }
                                        }
                                        ...
                                        for (int jj = 7; jj < 34; jj++)
                                        {
                                            ...
                                            var temptesteStyle = secondRow5.GetCell(1).CellStyle;
                                            IRow secondRow51 = reportSheet.GetRow(jj + rowNo);
                                            secondRow51.GetCell(1).CellStyle = temptesteStyle;
                                            var temptesteStyle1 = secondRow5.GetCell(0).CellStyle;
                                            secondRow51.GetCell(0).CellStyle = temptesteStyle1;
                                        }

                                        var cra = new NPOI.SS.Util.CellRangeAddress(rowNo, rowNo + 4, 0, 13);
                                        reportSheet.AddMergedRegion(cra);
                                        IRow Row1 = reportSheet.GetRow(0);
                                        var tempStyle = Row1.GetCell(0).CellStyle;
                                        IRow Row11 = reportSheet.GetRow(rowNo);
                                        Row11.GetCell(0).CellStyle = tempStyle;
                                        string CompanyName = Row1.GetCell(0).StringCellValue;
                                        Row11.GetCell(0).SetCellValue(CompanyName);

                                        var cra1 = new NPOI.SS.Util.CellRangeAddress(rowNo + 30, rowNo + 34, 0, 13);
                                        reportSheet.AddMergedRegion(cra1);
                                        IRow Row2 = reportSheet.GetRow(30);
                                        var tempStyle1 = Row2.GetCell(0).CellStyle;
                                        IRow Row21 = reportSheet.GetRow(rowNo + 30);
                                        Row21.GetCell(0).CellStyle = tempStyle1;
                                        string Notes = Row2.GetCell(0).StringCellValue;
                                        Row21.GetCell(0).SetCellValue(Notes);


                                        var cra2 = new NPOI.SS.Util.CellRangeAddress(rowNo + 35, rowNo + 38, 0, 3);
                                        reportSheet.AddMergedRegion(cra2);
                                        IRow Row3 = reportSheet.GetRow(35);
                                        var tempStyle2 = Row3.GetCell(0).CellStyle;
                                        IRow Row31 = reportSheet.GetRow(rowNo + 35);
                                        Row31.GetCell(0).CellStyle = tempStyle2;
                                        string Checked_By = Row3.GetCell(0).StringCellValue;
                                        Row31.GetCell(0).SetCellValue(Checked_By);

                                        var cra3 = new NPOI.SS.Util.CellRangeAddress(rowNo + 35, rowNo + 38, 4, 8);
                                        reportSheet.AddMergedRegion(cra3);
                                        IRow Row4 = reportSheet.GetRow(35);
                                        var tempStyle3 = Row4.GetCell(4).CellStyle;
                                        IRow Row41 = reportSheet.GetRow(rowNo + 35);
                                        Row41.GetCell(4).CellStyle = tempStyle3;
                                        string Verified_By = Row4.GetCell(4).StringCellValue;
                                        Row41.GetCell(4).SetCellValue(Verified_By);

                                        var cra4 = new NPOI.SS.Util.CellRangeAddress(rowNo + 35, rowNo + 38, 9, 13);
                                        reportSheet.AddMergedRegion(cra4);
                                        IRow Row5 = reportSheet.GetRow(35);
                                        var tempStyle4 = Row5.GetCell(9).CellStyle;
                                        IRow Row51 = reportSheet.GetRow(rowNo + 35);
                                        Row51.GetCell(9).CellStyle = tempStyle4;
                                        string Approved_By = Row5.GetCell(9).StringCellValue;
                                        Row51.GetCell(9).SetCellValue(Approved_By);

                                        IRow secondRow2 = reportSheet.GetRow(0);
                                        string cellValue = secondRow2.GetCell(0).StringCellValue;
                                        IRow secondRow21 = reportSheet.GetRow(rowNo);
                                        secondRow21.GetCell(0).SetCellValue(cellValue);

                                        for (int ii = 0; ii < 14; ii++)
                                        {
                                            var row4 = dataSheet.GetRow(7);
                                            IRow secondRow4 = reportSheet.GetRow(rowNo + 7);
                                            var tempValue4 = row4.GetCell(ii).StringCellValue;
                                            secondRow4.GetCell(ii).SetCellValue(tempValue4);
                                        }
                                        rowIncrement += 17;
                                    }
                                    rowIncrement = 17;
                                ...
                                for (int i = 0, loopTo1 = _extraInformationDataView.Rows.Count - 1; i <= loopTo1; i++)
                                {
                                    ...
                                    string Title_Col = Conversions.ToString(_extraInformationDataView.Rows[i].Cells[5].Value); // The column Information 
                                    string Title_Name = Conversions.ToString(_extraInformationDataView.Rows[i].Cells[2].Value);
                                    cell = sheet.GetRow(Title_Row).GetCell(GetColIndex(Title_Col));                     // The title's name 
                                    cell.SetCellValue(Title_Name);
                                    testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                                    cell.CellStyle = testeStyle;
                                    ...
                                    string Col_Next = GetColumnName(Next_Col_TitleValue + 1);
                                    string Title_Value = Conversions.ToString(_extraInformationDataView.Rows[i].Cells[4].Value);
                                    cell = sheet.GetRow(Title_Row).GetCell(GetColIndex(Col_Next));                     // The title's name 
                                    cell.SetCellValue(Title_Value);
                                    testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                                    cell.CellStyle = testeStyle;
                            ...
                            {
                                int CurrentUsedColumn = GetColIndex(InitialField_Column);
                                cell = sheet.GetRow(LastRowUsed - 1).CreateCell(CurrentUsedColumn - 1);
                                cell.SetCellValue((double)srNo + 1);
                                testeStyle = workbook.CreateCellStyle();
                                testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                                testeStyle.BorderLeft = NPOI.SS.UserModel.BorderStyle.Thin;
                                ...
                                for (int j = 0, loopTo7 = Fieldrows.Length - 1; j <= loopTo7; j++)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(Fieldrows[j][13].ToString()))
                                    {
                                        for (int i = 0, loopTo8 = MeasurementStringList.Count - 1; i <= loopTo8; i++)
                                        {
                                            ...
                                            if(i == MeasurementStringList.Count - 1)
                                            {
                                                count = 0;
                                            }
                                            if (Fieldrows[j][5].ToString().Contains(Conversions.ToString(degree)) && ToleranceFormat != 0)
                            ...
                            else
                            {
                                ...
                                for (int j = 0, loopTo4 = Fieldrows.Length - 1; j <= loopTo4; j++)
                                {
                                    ...
                                    //else
                                    //{
                                    //    cell = sheet.GetRow(Currentrow + 16).GetCell(GetColIndex(LastColumnUsed) - 1);
                                    //    cell.SetCellValue(srNo + 1);
                ...
                try
                {
                    if (!Fasttransfer)
                    {
                        ...
                        int P_index3 = ProgramsPath.LastIndexOf(@"\");
                        tmpStr = ProgramName.Substring(P_index + 1, P_index2 - P_index - 1);
                        DestinationTemplateFile = ProgramsPath.Substring(0, P_index3) + "\\" + "Templates" + "\\" + tmpStr + "_ReportStyle1_Template" + ".xls";

                        System.IO.FileInfo DestinationFileStatus1 = new System.IO.FileInfo(DestinationFile);
                        if (DestinationFileStatus1.Exists)
                        {
                            System.IO.FileInfo DestinationTemplateFileStatus = new System.IO.FileInfo(DestinationTemplateFile);
                            if (DestinationTemplateFileStatus.Exists)
                            {
                                DestinationTemplateFileStatus.Delete();
                            }
                            DestinationFileStatus1.CopyTo(DestinationTemplateFile);
                        }

                        using (FileStream TemplateStream = new FileStream(DestinationTemplateFile, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                        {
                            workbook = new HSSFWorkbook(TemplateStream);
                            sheet = workbook.GetSheet("Report");

                            using (FileStream TemplateStream2 = new FileStream(DestinationTemplateFile, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                            {
                                var Sheet = workbook.GetSheet("Report");
                                if (ReportType.Contains("Column"))
                                {
                                    ...
                                    var RowCellRange = CellRangeAddress.ValueOf(rowRange);
                                    for (var n = RowCellRange.FirstRow; n <= RowCellRange.LastRow; n++)
                                    {
                                        var row = Sheet.GetRow(n);

                                        for (var j = RowCellRange.FirstColumn; j <= RowCellRange.LastColumn; j++)
                                        {
                                            row.GetCell(j).SetCellValue("");
                                        }
                                    }
                                    if (MeasurementStringList.Count > 13)
                                    {
                                        string rowRange1;
                                        if (IncludeTolerence)
                                        {
                                        ...
                                        var RowCellRange1 = CellRangeAddress.ValueOf(rowRange1);
                                        for (var n = RowCellRange1.FirstRow; n <= RowCellRange1.LastRow; n++)
                                        {
                                            var row = Sheet.GetRow(n);

                                            for (var j = RowCellRange1.FirstColumn; j <= RowCellRange1.LastColumn; j++)
                                            {
                                                row.GetCell(j).SetCellValue("");
                                            }
                                        }
                                    }
                                    workbook.Write(TemplateStream2);
                                    workbook.Close();
        ...
        private void CreateDetailSheet()
        {
            ...
            for (int i = 0, loopTo1 = Tole_Headers.Count - 1; i <= loopTo1; i++)
            {
                ...
                cell = sheet.GetRow(InitialField_Row).GetCell(GetColIndex(Col));
                cell.SetCellValue(Tole_Headers[i]);
                testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                testeStyle.FillPattern = FillPattern.SolidForeground;
                testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                cell.CellStyle = testeStyle;
        ...
        {
            sheet = workbook.CreateSheet("RawDataSheet");
            if (RawDataSheetType.Contains("Column"))
            {
                int a = 1;
                cell = sheet.CreateRow(1).CreateCell(a);
        ...
        private void WriteDetailInfo()
        {
            ...
            {
                Dno = Conversions.ToInteger(Operators.DivideObject(Operators.SubtractObject(workbook.NumberOfSheets, 2), 2));
            }
            else
            {
                Dno = Conversions.ToInteger(Operators.SubtractObject(workbook.NumberOfSheets, 2));
            }

            string outoftoldeg;
            double outoftol;
            sheet = workbook.GetSheetAt(1);
            sheet.CopySheet("DetailSheet" + Dno);
            CellRangeDelete(TemplateFinalRange);
            ...
            for (int j = 0, loopTo = Fieldrows.Length - 1; j <= loopTo; j++)
            {
                for (int i = 0, loopTo1 = MeasurementStringList.Count - 1; i <= loopTo1; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(Fieldrows[j][13].ToString()) & Fieldrows[j][3].ToString() != "Date" & Fieldrows[j][3].ToString() != "Time")
                    {
                        ...
                        cell = sheet.GetRow(Currentrow).GetCell(a);
                        testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                        testeStyle.FillPattern = FillPattern.SolidForeground;
                        cell.CellStyle = testeStyle;
                        ...
                        if (!string.IsNullOrEmpty(Fieldrows[j][7].ToString()))
                        {
                            ...
                            {
                                cell = sheet.GetRow(Currentrow).GetCell(a + 5);
                                cell.SetCellValue((double)MeasurementStringList[i][2] - (double)Fieldrows[j][7]);
                                if (cell.StringCellValue.Contains("-"))
                                {
                                    outoftol = Conversions.ToDouble(Operators.SubtractObject(Conversions.ToDouble(cell.StringCellValue), Fieldrows[j][9]));
                                    ...
                                    {
                                        cell = sheet.GetRow(Currentrow).GetCell(a + 6);
                                        testeStyle.FillForegroundColor = IndexedColors.Orange.Index;
                                        testeStyle.FillPattern = FillPattern.SolidForeground;
                                        cell.CellStyle = testeStyle;
                                    }
                                    cell = sheet.GetRow(Currentrow).GetCell(a + 6);
                                    cell.SetCellValue(outoftol);
                                }
                                else
                                {
                                    cell = sheet.GetRow(Currentrow).GetCell(a + 5);
                                    outoftol = Conversions.ToDouble(Operators.SubtractObject(Conversions.ToDouble(cell.StringCellValue), Fieldrows[j][8]));
                                    if (outoftol.ToString().Contains("-"))
                                    {
                                        outoftol = 0.0d;
                                    }
                            ...
                            {
                                cell = sheet.GetRow(Currentrow).GetCell(a + 5);
                                cell.SetCellValue(chgeform((ConvertDegMinSecToDecimal(Conversions.ToString(MeasurementStringList[i][2])) - ConvertDegMinSecToDecimal(Conversions.ToString(Fieldrows[j][7]))).ToString(), false));
                                if (cell.StringCellValue.Contains("-"))
                                {
                                    outoftoldeg = this.chgeform((ConvertDegMinSecToDecimal(Conversions.ToString(cell.StringCellValue)) - ConvertDegMinSecToDecimal(Conversions.ToString(Fieldrows[j][9]))).ToString(), false);
                                    ...
                                    cell = sheet.GetRow(Currentrow).GetCell(a + 6);
                                    cell.SetCellValue(outoftoldeg);
                                }
                                else
                                {
                                    cell = sheet.GetRow(Currentrow).GetCell(a + 5);
                                    outoftoldeg = this.chgeform((ConvertDegMinSecToDecimal(Conversions.ToString(cell.StringCellValue)) - ConvertDegMinSecToDecimal(Conversions.ToString(Fieldrows[j][8]))).ToString(), false);
                                    if (outoftoldeg.ToString().Contains("-"))
                                    {
                                        outoftoldeg = 0.ToString() + degree;
                                    }
        ...
        {
            sheet = workbook.GetSheetAt(1);
            sheet.CopySheet("RawDataSheet");
            if (RawDataSheetType.Contains("Column"))
            {
                int a = 1;
                cell = sheet.GetRow(ComponentNo + 1).GetCell(a);
                testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                cell.CellStyle = testeStyle;
                a += 1;
                for (int j = 0, loopTo = Fieldrows.Length - 1; j <= loopTo; j++)
            ...
            {
                int Currentrow = 1;
                cell = sheet.GetRow(Currentrow).GetCell(ComponentNo + 1);
                testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                cell.CellStyle = testeStyle;
                Currentrow += 1;
        ...
        public void CompareAndWrite_FieldValues(string FieldValue, int FieldValue_Row, string FieldValue_Column, bool WritePassFailFirstTime = false, int CurrentIndex = 0, int NumberFormat = -1)
        {
            try
            {
                cell = sheet.GetRow(FieldValue_Row - 1).CreateCell(GetColIndex(FieldValue_Column) - 1);
                if (ReportType.Contains("Column"))
                {
                    if (GetColIndex(FieldValue_Column) == 2 || GetColIndex(FieldValue_Column) == 3)
                    {
                        if (cell.StringCellValue == "")
                        {
                            cell.SetCellValue(FieldValue);
                            sheet.AutoSizeColumn(GetColIndex(FieldValue_Column) - 1);
                        }
                    }
                }
                else
                {
                    if (FieldValue_Row == 9 || FieldValue_Row == 10)
                    {
                        if (cell.StringCellValue == "")
                        {
                            cell.SetCellValue(FieldValue);
                ...
                double FinalFieldValues;
                bool ParsedFinalFieldValues = double.TryParse(FieldValue, out FinalFieldValues);
                if (!ParsedFinalFieldValues)
                {
                    if(IncludeTolerence && FieldValue.Substring(FieldValue.Length - 2,2)== "''")
                    {
                        var angleValue = double.Parse(FieldValue.Substring(0, FieldValue.Length - 10) + FieldValue.Substring(FieldValue.Length - 8,2) + FieldValue.Substring(FieldValue.Length - 4,2));
                        if (angleValue < minimumValue[count] || FinalFieldValues > maximumValue[count])
                        {
                            cell.SetCellValue(FieldValue);
                            ICellStyle testeStyleRed = workbook.CreateCellStyle();
                            testeStyleRed.CloneStyleFrom(toleranceCellStyleRed);
                            cell.CellStyle = testeStyleRed;
                        }
                        else
                        {
                            cell.SetCellValue(FieldValue);
                            ICellStyle testeStyleGreen = workbook.CreateCellStyle();
                            testeStyleGreen.CloneStyleFrom(toleranceCellStyleGreen);
                            cell.CellStyle = testeStyleGreen;
                        }
                        count++;
                    }
                    else
                ...
                else
                {
                    if(IncludeTolerence)
                    {
                        if(FinalFieldValues < minimumValue[count] || FinalFieldValues > maximumValue[count])
                        {
                            if (minimumValue[count] == 0 && maximumValue[count] == 0)
                            {
                                cell.SetCellValue(Math.Round(FinalFieldValues, 4));
                            }
                            else
                            {
                                cell.SetCellValue(Math.Round(FinalFieldValues, 4));
                ...
                testeStyle = workbook.CreateCellStyle();
                testeStyle.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Right;
                //cell.CellStyle = testeStyle;
                sheet.AutoSizeColumn(GetColIndex(FieldValue_Column) - 1);
            }
            catch (Exception ex)
        ...
        private void CellRangeDelete(string Range)
        {
            var cellRange = CellRangeAddress.ValueOf(Range);
        ...
        private void CellRangeFormula(string Range, string Value)
        {
            var cellRange = CellRangeAddress.ValueOf(Range);
            ...
            try
            {
                ...
                if (WriteHeaders)
                {
                    ...
                    if (ShwfieldNum)
                    {
                        if (ReportType.Contains("Column"))
                        {
                            cell = sheet.GetRow(Tole_Row).GetCell(GetColIndex(Tole_Column));
                            cell.SetCellValue("M.No.");

                            cell = sheet.GetRow(Tole_Row + 1).GetCell(GetColIndex(Tole_Column));
                            cell.SetCellValue("S.No.");
                            testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                            testeStyle.FillPattern = FillPattern.SolidForeground;
                            cell.CellStyle = testeStyle;
                        ...
                        else
                        {
                            ...
                            cell = sheet.GetRow(Tole_Row).GetCell(GetColIndex(Tole_Column + 1));
                            cell.SetCellValue("S.No.");
                            testeStyle.FillForegroundColor = IndexedColors.LightBlue.Index;
                            testeStyle.FillPattern = FillPattern.SolidForeground;
                            cell.CellStyle = testeStyle;
                    ...
                    if (IncludeTolerence & ToleranceFormat != 0)
                    {
                        ...
                        if (ReportType.Contains("Column"))
                        {
                            for (int i = 0, loopTo = Tole_Headers.Count; i < loopTo; i++)
                            {
                                //XSSFWorkbook workbook1 = new XSSFWorkbook();
                                cell = sheet.GetRow(Tole_Row + i).GetCell(GetColIndex(Tole_Column) - 1);
                                cell.SetCellValue(Tole_Headers[i]);
                                ICellStyle testeStyle1;
                                testeStyle1 = workbook.CreateCellStyle();
                                testeStyle1.CloneStyleFrom(toleranceCellStyleBorder);
                                cell.CellStyle = testeStyle1;
                                sheet.AutoSizeColumn(GetColIndex(Tole_Column) - 1);
                            }
                        }
                        else
                        {
                            for (int i = 0, loopTo1 = Tole_Headers.Count; i < loopTo1; i++)
                            {
                                //string Col = GetColumnName(GetColIndex(Tole_Column) + i);
                                cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) + i);
                                cell.SetCellValue(Tole_Headers[i]);
                                ICellStyle testeStyle1;
                                testeStyle1 = workbook.CreateCellStyle();
                                testeStyle1.CloneStyleFrom(toleranceCellStyleBorderRow);
                                cell.CellStyle = testeStyle1;
                                //sheet.AutoSizeColumn(GetColIndex(Tole_Column) + i);
                            }
                        }
                    }
                    ...
                    cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) - 1);
                    cell.SetCellValue("S.No.");
                    ...
                    for (int i = 0, loopTo3 = MeasurementStringList.Count - 1; i <= loopTo3; i++) // ' _measurementListView.Rows.Count - 1
                    {
                        ...
                        if (ReportType.Contains("Column"))
                        {
                            if (GetColIndex(Tole_Column) - 1 == GetColIndex(ColumnAfterWhich_PaginationShouldBeDone))
                            {
                                ...
                                Tole_Column = GetColumnName(GetColIndex(Tole_Column));
                                templateno += 1;
                            }
                        }
                        else if (ReportType.Contains("Row"))
                        ...
                        if (ReportT.Contains("Column"))
                        {
                            ...
                            {
                                cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column));
                                cell.SetCellValue((double)MeasurementStringList[i][6]);
                        ...
                        if (ReportType.Contains("Column"))
                        {
                            if (Tole_Column == "O" || Tole_Column == "AC" || Tole_Column == "AQ" || Tole_Column == "BE" || Tole_Column == "BS")
                            {
                                cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) - 1);
                                string tempHeading = "S.No.";
                                cell.SetCellValue(tempHeading);
                                Tole_Column = GetColumnName(GetColIndex(Tole_Column) + 1);
                            }
                        }
                        //if (ReportType.Contains("Row"))
                        //{
                        //    if (Tole_Row == 32)
                        //    {
                        //        cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) - 1);
                        ...
                        cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) - 1);
                        string temp = (string)MeasurementStringList[i][0];
                        ...
                        if (IncludeTolerence & ToleranceFormat != 0) // ' And
                        {
                            ...
                            if (IsDegree)
                            {
                                ...
                                HigherT = USL;
                                LowerT = LSL;
                                var minValue = min.ToString().Substring(0, min.ToString().Length - 8) + min.ToString().Substring(min.ToString().Length - 7, 2) + min.ToString().Substring(min.ToString().Length - 4, 2);
                                var maxValue = max.ToString().Substring(0, max.ToString().Length - 8) + max.ToString().Substring(max.ToString().Length - 7, 2) + max.ToString().Substring(max.ToString().Length - 4, 2);
                                minimumValue[i] = double.Parse(minValue);
                                maximumValue[i] = double.Parse(maxValue);
                            }
                            else
                            {
                                HigherT = USL;
                                ...
                                Nomi = Conversions.ToDouble(Nomi).ToString();
                                minimumValue[i] = (double)min;
                                maximumValue[i] = (double)max;
                            }
                                ...
                                if (ReportT.Contains("Column"))    // ------------------------- for columnwise reporting--------------------
                                {
                                    if (i > 1)
                                    {
                                        ...
                                        if (WriteToleranceEntries)
                                        {
                                            ...
                                            {
                                                cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) - 1);
                                                cell.SetCellValue(Conversions.ToString(min) + " -- " + Conversions.ToString(max));
                                                sheet.AutoSizeColumn(GetColIndex(Tole_Column) - 1);
                                            }
                                            else
                                            {
                                                ...
                                                if (!LowerT.Contains("-"))
                                                    LowerT = "+" + LowerT;
                                                cell = sheet.GetRow(Tole_Row - 1).GetCell(GetColIndex(Tole_Column) - 1);
                                                var NomiLength = Nomi.Length;
                                                var HigherTLength = HigherT.Length;
                                                var LowerTLength = LowerT.Length;
                                                HSSFFont superscript = (HSSFFont)workbook.CreateFont();
                                                superscript.TypeOffset = FontSuperScript.Super;//Superscript
                                                HSSFFont subscript = (HSSFFont)workbook.CreateFont();
                                                subscript.TypeOffset = FontSuperScript.Sub;
                                                HSSFRichTextString tempValue;
                                                tempValue = new HSSFRichTextString(Nomi + " " + HigherT + " " + LowerT);
                                                tempValue.ApplyFont(NomiLength + 1, NomiLength + 1 + HigherTLength, superscript);
                                                tempValue.ApplyFont(NomiLength + 2 + HigherTLength, NomiLength + 2 + HigherTLength + LowerTLength, subscript);
                                        ...
                                        //CellRangeDelete(Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert);
                                        if (!IsDegree)
                                        {
                                            if (WriteToleranceEntries)
                                            {
                                                //{
                                                //    SCF = sheet.SheetConditionalFormatting;
                                                //    ColorFormat = SCF.CreateConditionalFormattingRule(ComparisonOperator.Between, "");
                                                //    FillColor = ColorFormat.CreatePatternFormatting();
                                                //    FillColor.FillBackgroundColor = 35;
                                                //    FillColor.FillPattern = FillPattern.SolidForeground;
                                                //    CellRangeAddress[] Range = { CellRangeAddress.ValueOf(Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert) };
                                                //    SCF.AddConditionalFormatting(Range, ColorFormat);
                                                //}

                                                //{
                                                //    SCF = sheet.SheetConditionalFormatting;
                                                //    ColorFormat = SCF.CreateConditionalFormattingRule(ComparisonOperator.Between, (string)min, (string)max);
                                                //    FillColor = ColorFormat.CreatePatternFormatting();
                                                //    FillColor.FillBackgroundColor = IndexedColors.LightBlue.Index;
                                                //    FillColor.FillPattern = FillPattern.SolidForeground;
                                                //    CellRangeAddress[] Range = { CellRangeAddress.ValueOf(Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert) };
                                                //    SCF.AddConditionalFormatting(Range, ColorFormat);
                                                //}
                                                ...
                                                //{
                                                //    SCF = sheet.SheetConditionalFormatting;
                                                //    ColorFormat = SCF.CreateConditionalFormattingRule(ComparisonOperator.NotBetween, (string)min, (string)max);
                                                //    FillColor = ColorFormat.CreatePatternFormatting();
                                                //    FillColor.FillBackgroundColor = IndexedColors.Orange.Index;
                                                //    FillColor.FillPattern = FillPattern.SolidForeground;
                                                //    CellRangeAddress[] Range = { CellRangeAddress.ValueOf(Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert) };
                                                //    SCF.AddConditionalFormatting(Range, ColorFormat);
                                                //}
                                            ...
                                            {
                                                {
                                                    var range = Tole_Column + (RowToInsert + 1) + ":" + Tole_Column + (RowToInsert + 1);
                                                    var formula = "AVERAGE(" + Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert + ")";
                                                    CellRangeFormula(range, formula);

                                                }

                                                {
                                                    var range = Tole_Column + (RowToInsert + 2) + ":" + Tole_Column + (RowToInsert + 2);
                                                    var formula = "MIN(" + Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert + ")";
                                                    CellRangeFormula(range, formula);

                                                }

                                                {
                                                    var range = Tole_Column + (RowToInsert + 3) + ":" + Tole_Column + (RowToInsert + 3);
                                                    var formula = "MAX(" + Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + RowToInsert + ")";
                                                    CellRangeFormula(range, formula);

                                                }

                                                {
                                                    var range = Tole_Column + (RowToInsert + 4) + ":" + Tole_Column + (RowToInsert + 4);
                                                    var formula = Tole_Column + (RowToInsert + 3) + " - " + Tole_Column + (RowToInsert + 2);
                                                    CellRangeFormula(range, formula);
                                ...
                                else    // ---------------------------------- for row wise reporting------------------------------------------------------------
                                {
                                    if (i > 1)
                                    {
                                        ...
                                        if (WriteToleranceEntries)
                                        {
                                            if (ToleranceF == 1)
                                            {
                                                cell = sheet.GetRow(Tole_Row - 1).GetCell(GetIndex);
                                                cell.SetCellValue(Conversions.ToString(min) + " -- " + Conversions.ToString(max));
                                                sheet.AutoSizeColumn(GetColIndex(Tole_Column));
                                            }
                                            else
                                            {
                                                if (!HigherT.Contains("-"))
                                        ...
                                        CellRangeDelete(Tole_Column + Tole_Row + ":" + GetColumnName(ColToInsert) + Tole_Row);
                                        ...
                                        if (!IsDegree)
                                        {
                                            if (WriteToleranceEntries)
                                            {
                                                //{
                                                //    SCF = sheet.SheetConditionalFormatting;
                                                //    ColorFormat = SCF.CreateConditionalFormattingRule(ComparisonOperator.NoComparison, null);
                                                //    FillColor = ColorFormat.CreatePatternFormatting();
                                                //    FillColor.FillBackgroundColor = IndexedColors.White.Index;
                                                //    FillColor.FillPattern = FillPattern.SolidForeground;
                                                //    CellRangeAddress[] Range = { CellRangeAddress.ValueOf(Tole_Column + Tole_Row + ":" + GetColumnName(ColToInsert) + Tole_Row) };
                                                //    SCF.AddConditionalFormatting(Range, ColorFormat);
                                                //}
                                            ...
                                            {
                                                {
                                                    var formula = "AVERAGE(" + Tole_Column + Tole_Row + ":" + GetColumnName(ColToInsert) + Tole_Row + ")";
                                                    cell = sheet.GetRow(Tole_Row - 1).GetCell(ColToInsert);
                                                    cell.SetCellFormula(formula);
                                                }

                                                {
                                                    var formula = "MAX(" + Tole_Column + Tole_Row + ":" + GetColumnName(ColToInsert) + Tole_Row + ")";
                                                    cell = sheet.GetRow(Tole_Row - 1).GetCell(ColToInsert + 1);
                                                    cell.SetCellFormula(formula);
                                                }

                                                {
                                                    var formula = "MIN(" + Tole_Column + Tole_Row + ":" + GetColumnName(ColToInsert) + Tole_Row + ")";
                                                    cell = sheet.GetRow(Tole_Row - 1).GetCell(ColToInsert + 2);
                                                    cell.SetCellFormula(formula);
                                                }

                                                {
                                                    var formula = GetColumnName(ColToInsert + 2) + Tole_Row + " - " + GetColumnName(ColToInsert + 3) + Tole_Row;
                                                    cell = sheet.GetRow(Tole_Row - 1).GetCell(ColToInsert + 3);
                                                    cell.SetCellFormula(formula);
                                                }
                                            }
                                        }
                            ...
                            else if (ToleranceF == 3)                 // If LSL,USL format is selected.
                            {
                                if (ReportT.Contains("Column")) // ------------------------- for columnwise reporting--------------------
                                {
                                    if (i > 1)
                                    {
                                        ...
                                        {
                                            cell = sheet.GetRow(Tole_Row).GetCell(GetColIndex(Tole_Column) - 1);
                                            cell.SetCellValue(double.Parse(Nomi));
                                            cell = sheet.GetRow(Tole_Row + 1).GetCell(GetColIndex(Tole_Column) - 1);
                                            cell.SetCellValue((double)min);
                                            cell = sheet.GetRow(Tole_Row + 2).GetCell(GetColIndex(Tole_Column) - 1);
                                            cell.SetCellValue((double)max);
                                            sheet.AutoSizeColumn(GetColIndex(Tole_Column) - 1);
                                        }
                                        ...
                                        if (!IsDegree)
                                        {
                                            ...
                                            if (ShowAvging)
                                            {
                                                {
                                                    var range = Tole_Column + (RowToInsert - 2) + ":" + Tole_Column + (RowToInsert - 2);
                                                    var formula = "AVERAGE(" + Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + (RowToInsert - 3)+ ")";
                                                    CellRangeFormula(range, formula);
                                                ...
                                                {
                                                    var range = Tole_Column + (RowToInsert -1) + ":" + Tole_Column + (RowToInsert -1);
                                                    var formula = "MIN(" + Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + (RowToInsert - 3) + ")";
                                                    CellRangeFormula(range, formula);

                                                }

                                                {
                                                    var range = Tole_Column + RowToInsert + ":" + Tole_Column + RowToInsert;
                                                    var formula = "MAX(" + Tole_Column + (Tole_Row + 1) + ":" + Tole_Column + (RowToInsert - 3) + ")";
                                                    CellRangeFormula(range, formula);
                                                ...
                                                {
                                                    var range = Tole_Column + (RowToInsert + 1) + ":" + Tole_Column + (RowToInsert + 1);
                                                    var formula = Tole_Column + (RowToInsert -1) + " - " + Tole_Column + RowToInsert;
                                                    CellRangeFormula(range, formula);
                                ...
                                else  // ---------------------------------- for row wise reporting------------------------------------------------------------
                                {
                                    if (i > 1)
                                    {
                                        ...
                                        if (WriteToleranceEntries)
                                        {
                                            cell = sheet.GetRow(Tole_Row - 1).GetCell(GetIndex);
                                            cell.SetCellValue(double.Parse(Nomi));
                                            cell = sheet.GetRow(Tole_Row - 1).GetCell(GetIndex + 1);
                                            cell.SetCellValue((double)min);
                                            cell = sheet.GetRow(Tole_Row - 1).GetCell(GetIndex + 2);
                                            cell.SetCellValue((double)max);
                                        }
                        ...
                        if (passStatus != PassFail.none)
                        {
                            if (ReportT.Contains("Column"))
                            {
                                cell = sheet.CreateRow(Tole_Row).CreateCell(GetColIndex(Tole_Column + 1));
