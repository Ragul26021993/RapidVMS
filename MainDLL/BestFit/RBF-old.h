...
#pragma endregion
			...
			bool Torus_BestFit(double* pts, int PtsCount, double* answer);
			...
			bool setRefImage (BYTE* pixelArray, int width, int height, bool singlechannel, int deadPixelWidth = 10, int binarizingCutoff = 200, int scale = 1, bool recalculateLRdeadPixWidth = false);
			...
			int RedefineImagesAboutCG (bool both, int* p_xy, int tol_error = 5);
			...
			int getMaxImageMatch (double* p_Theta, bool singlechannel, int tol_error_ll = -1);
