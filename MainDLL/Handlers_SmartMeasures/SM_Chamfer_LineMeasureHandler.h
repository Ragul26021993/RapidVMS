...
class SM_Chamfer_LineMeasureHandler:
	public MouseHandler
{
private:
	...
	void AddPt2PtDistanceMeasure(Vector* Pt1, Vector* Pt2, double* MousePt);
	void AddLine2Line_AngleMeasure(Shape* CShape1, Shape* CShape2, double* MousePt);

public:
	SM_Chamfer_LineMeasureHandler(bool InitialiseShape = true); //default constructor..
	~SM_Chamfer_LineMeasureHandler(); //destructor... clear the memory related..!
	//below is for having chamfer distance measurement called from line-arc action class
	void call_chamfer_dist_measurement(Shape* csh1, Shape* csh2, Shape* csh3);
