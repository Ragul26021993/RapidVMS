...
//Default constuctor..
SM_AngleMeasureWithYAxis::SM_AngleMeasureWithYAxis()
{
	try
	{
		FirstTime = true;
		...
		MAINDllOBJECT->SetStatusCode("SM_AngleMeasureWithYAxis01");
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0001", __FILE__, __FUNCSIG__); }
}
...
SM_AngleMeasureWithYAxis::~SM_AngleMeasureWithYAxis()
{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0002", __FILE__, __FUNCSIG__); }
}

//Initialise default settings, parameters etc.(flags, Mouse clicks required..)
void SM_AngleMeasureWithYAxis::init()
{
	try
	{
		AngleMeasureQuad = 0;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0003", __FILE__, __FUNCSIG__); }
}

//Initialise handler for New Angle Measure..!!
void SM_AngleMeasureWithYAxis::InitialiseNewAngleMeasure()
{
	try
	{
		if(FirstTime)
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0004", __FILE__, __FUNCSIG__); }
}

//Add New Angle measurement
void SM_AngleMeasureWithYAxis::AddLine_AngleMeasurement()
{
	try
	{
		FirstTime = false;
		...
		((DimLineToLineDistance*)CurrentMeasure)->AngleTypeWithAxis = DimLineToLineDistance::ANGLEWITHAXIS::ANGLE_YAXIS;
		CurrentMeasure->IsValid(false);
		CurrentMeasure->UpdateMeasurement();
		AddDimAction::addDim(CurrentMeasure);
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0005", __FILE__, __FUNCSIG__); }
}

//Handle the mouse move and update the current angle measurement..
void SM_AngleMeasureWithYAxis::AngleMeasurement_MouseMove()
{
	try
	{
		...
		if(abs(langle1 - langle2) > 0.00001)
			RMATH2DOBJECT->Line_lineintersection(Cshape2->Angle(), Cshape2->Intercept(), Cshape1->Angle(), Cshape2->getPoint1()->getX(), &ptonarc[0]);
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0006", __FILE__, __FUNCSIG__); }
}

//Set the Measurement Position when Flexible / FG is used 
void SM_AngleMeasureWithYAxis::SetMeasurementPosition()
{
	try
	{
		...
		else
		{
			...
			if(r_angle1 < M_PI)
				mid_angle = (r_angle1 + M_PI_2)/2;
			else
				mid_angle = (r_angle1 + 3 * M_PI_2)/2;
...
void SM_AngleMeasureWithYAxis::ChangeHanlder_Type(RapidEnums::RAPIDHANDLERTYPE CHandle)
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0008", __FILE__, __FUNCSIG__); }
}

//Function to handle the mouse move..
void SM_AngleMeasureWithYAxis::mouseMove()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0009", __FILE__, __FUNCSIG__); }
}

//Left mouse down handling
void SM_AngleMeasureWithYAxis::LmouseDown()
{	
	try
	{
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0010", __FILE__, __FUNCSIG__); }
}

//Left Mouse Up handling
void SM_AngleMeasureWithYAxis::LmouseUp(double x, double y)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0011", __FILE__, __FUNCSIG__); }
}

//Handle Mouse scroll..
void SM_AngleMeasureWithYAxis::MouseScroll(bool flag)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0012", __FILE__, __FUNCSIG__); }
}

//Handle right mouse down... Add new line/ initialise new measurement..
void SM_AngleMeasureWithYAxis::RmouseDown(double x, double y)
{
	try
	{
		if(MeasurementAdded)
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0013", __FILE__, __FUNCSIG__); }
}

//Handle all the conditions for the right click
void SM_AngleMeasureWithYAxis::CheckRightClickConditions()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0015", __FILE__, __FUNCSIG__); }
}

//Hanlde the left mouse down.. Last click for the current handler..
void SM_AngleMeasureWithYAxis::LmaxmouseDown()
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0016", __FILE__, __FUNCSIG__); }
}

//Check all the required conditions for the mouse down..
void SM_AngleMeasureWithYAxis::CheckAllTheConditions()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0017", __FILE__, __FUNCSIG__); }
}

//Escape buttons press handling..
void SM_AngleMeasureWithYAxis::EscapebuttonPress()
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0018", __FILE__, __FUNCSIG__); }
}

//draw Functions.. draw the flexible crosshair at mouse position..
void SM_AngleMeasureWithYAxis::draw(int windowno, double WPixelWidth)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("ANGM_YMH0019", __FILE__, __FUNCSIG__); }
}