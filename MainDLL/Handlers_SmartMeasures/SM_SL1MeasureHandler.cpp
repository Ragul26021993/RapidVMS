...
//Default constuctor..
SM_SL1MeasureHandler::SM_SL1MeasureHandler(bool withaxis)
{
	try
	{
		FirstTime = true;
		...
		MAINDllOBJECT->SetStatusCode("SM_SL1MeasureHandler01");
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("SL1MEASUREMH0001", __FILE__, __FUNCSIG__); }
}

//Destructor.. Release the memory here!
SM_SL1MeasureHandler::~SM_SL1MeasureHandler()
{
	try
	{
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("SL1MEASUREMH0002", __FILE__, __FUNCSIG__); }
}

//Initialise default settings, parameters etc.(flags, Mouse clicks required..)
void SM_SL1MeasureHandler::init()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("SL1MEASUREMH0003", __FILE__, __FUNCSIG__); }
}

//Initialise handler for New Angle Measure..!!
void SM_SL1MeasureHandler::InitialiseNewDistMeasure()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("SL1MEASUREMH0004", __FILE__, __FUNCSIG__); }
}

void SM_SL1MeasureHandler::AddShapeMeasureRelationShip(Shape* CShape)
{
	try
	{
		if(CShape != NULL)
...
void SM_SL1MeasureHandler::AddDistanceMeasurement()
{
	try
	{
		FirstTime = false;
...
void SM_SL1MeasureHandler::ChangeHanlder_Type(RapidEnums::RAPIDHANDLERTYPE CHandle)
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("SL1MEASUREMH0008", __FILE__, __FUNCSIG__); }
}

//Function to handle the mouse move..
void SM_SL1MeasureHandler::mouseMove()
{
	try
	{
...
void SM_SL1MeasureHandler::LmouseDown()
{	
	try
	{
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("SL1MEASUREMH0010", __FILE__, __FUNCSIG__); }
}

//Left Mouse Up
void SM_SL1MeasureHandler::LmouseUp(double x, double y)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
...
//Middle Mouse down..
void SM_SL1MeasureHandler::MmouseDown(double x, double y)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
...
//Mouse Scroll
void SM_SL1MeasureHandler::MouseScroll(bool flag)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
...
void SM_SL1MeasureHandler::RmouseDown(double x, double y)
{
	try
	{
		if(CurrentdrawHandler  != NULL)
...
void SM_SL1MeasureHandler::CheckRightClickConditions()
{
	try
	{
...
void SM_SL1MeasureHandler::LmaxmouseDown()
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
...
void SM_SL1MeasureHandler::CheckAllTheConditions()
{
	try
	{
...
void SM_SL1MeasureHandler::RotationStopedAddMeasure()
{
		if(CurrentMeasurementShCount == 2)
		{
			if(CurrentHandlerType == RapidEnums::RAPIDHANDLERTYPE::FLEX_CROSSHAIR)
...
void SM_SL1MeasureHandler::EscapebuttonPress()
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
...
void SM_SL1MeasureHandler::draw(int windowno, double WPixelWidth)
{
	try
	{
		if(CurrentdrawHandler == NULL) return;
