...
//Constructor..//
LinePerpendicularTangent::LinePerpendicularTangent()
{
	try
	{ 
		init(); 
		MAINDllOBJECT->SetStatusCode("LinePerpendicularTangent01");
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0001", __FILE__, __FUNCSIG__); }
}
...
LinePerpendicularTangent::~LinePerpendicularTangent()
{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0002", __FILE__, __FUNCSIG__); }
}

void LinePerpendicularTangent::ResetShapeHighlighted()
{
	try
	{
		if(ParentLine != NULL)
...
void LinePerpendicularTangent::init()
{
	try
	{
		setMaxClicks(3);
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0003", __FILE__, __FUNCSIG__); }
}

//Handling Left mousedown...//
void LinePerpendicularTangent::LmouseDown()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0004", __FILE__, __FUNCSIG__); }
}

//Handle the mouse move..//
void LinePerpendicularTangent::mouseMove()
{
	try
	{
		double mp[2] = {getClicksValue(getClicksDone()).getX(), getClicksValue(getClicksDone()).getY()};
		if(getClicksDone() == getMaxClicks() - 1)
			RMATH2DOBJECT->Tangent_Perpendicular(&Ccenter[0], radius, angle1, intercept1, LineLength, &mp[0], &point1[0], &point2[0], linepos);
	}
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0005", __FILE__, __FUNCSIG__); }
}

//draw perpendicular line..
void LinePerpendicularTangent::draw(int windowno, double WPixelWidth)
{
	try
	{
		if(getClicksDone() == getMaxClicks() - 1)
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0006", __FILE__, __FUNCSIG__); }
}

///Handle the mouse scroll... calculate the line end points for change in length...//
void LinePerpendicularTangent::MouseScroll(bool flag)
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0008", __FILE__, __FUNCSIG__); }
}

//Handle the partprogram data...//
void LinePerpendicularTangent::PartProgramData()
{
	try
	{
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0010", __FILE__, __FUNCSIG__); }
}

//Reset the current handler settings.,
void LinePerpendicularTangent::EscapebuttonPress()
{
	ResetShapeHighlighted();
	init(); 
}
...
void LinePerpendicularTangent::LmaxmouseDown()
{
	try
	{
		...
		((Line*)CurrentShape)->LineType = RapidEnums::LINETYPE::PERPENDICULAR_TANGENTL;
	...
	catch(...){ MAINDllOBJECT->SetAndRaiseErrorMessage("LINEPETMH0012", __FILE__, __FUNCSIG__); }
}

void LinePerpendicularTangent::SetAnyData(double *data)
{
	if(data[0] != 0)
	{
		LengthDefined = true;
...
void LinePerpendicularTangent::RmouseDown(double x, double y)
{
	try
	{
